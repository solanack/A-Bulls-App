...
export function AppShell() {
  ...
  return (
    ...
    >
        ...
        <header className="gz-top">
          <div className="gz-menu-wrap">
            <button
              ...
              aria-label="Open Galaxy Zero menu"
            ...
            {menuOpen ? (
              <nav className="gz-menu" aria-label="Galaxy Zero modes">
                ...
                <p className="gz-menu-status" title={dataStatus.disclosure}>
                  {dataStatus.store === "d1" ? "INDEXED" : "PROTOTYPE"} ·{" "}
      ...
      <section
        className={`field-shell__workspace${timelineOpen ? " field-shell__workspace--timeline" : ""}`}
      ...
      >
        <div className="field-shell__workspace-head">
          ...
          <button
            type="button"
            className="field-shell__close"
            onClick={() => osRef.current?.setMode("explore")}
          >
            RETURN TO FIELD
          </button>
        </div>
  ...
  if (!result) {
    return (
      <article className="workspace-copy">
        ...
        <p>
          ASK a public identifier on the Field. The same particles become the Grey and speak only
