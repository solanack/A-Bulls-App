...
import { anchorBolts, buildCutManifest, candleSource, candleSourceLabel, cohortTicks, explorerUrl, formatUsdNotional, formatUsdPrice, fullTape, groupBolts, hopSchedule, replayShareUrl, replaySubjectFrom, replayToolInput, STRIKE_MS, tapeAvgEntryMarketCap, tapeAxis, tapeCandles, tapeEvents, tapeEvidenceLine, tapeHeaderLine, tapeMarketCapAt, tapeMarketCapPoints, tapeMarkSummary, tapeScaleFor, tapeUsdCoverage, tapeUsdSummary, tapeWindow, type CohortTick, type CutFormat, type ReplaySubject, type TapeBolt, type TapeCandle, type TapeMarkSummary, type TapeMarketCapPoint, type TapeUsdSummary } from "@/lib/field/replay-tape";
...
export function ReplayStudio({ muted, onBack, onOpenRoom, watchlist = [], onToggleWatch }: { muted: boolean; onToggleMute?: () => void; onBack: () => void; onOpenRoom: (room: "fomo" | "afterbell") => void; watchlist?: readonly WatchItem[]; onToggleWatch?: (request: ReplayWatchRequest) => void }) {
  ...
  drawRef.current = () => {
    ...
    hitsRef.current = (globalThis as typeof globalThis & { __ABULLS_BOLTS?: BoltHit[] }).__ABULLS_BOLTS = drawTape(ctx, { width: size.w, height: size.h, candles, bolts, cohort: cohortOn && ticks.length ? ticks : undefined, marketCapUsd, mark, heroGlyph: subject ? (subject.displayName ?? handle ?? callsign(subject.wallet)) : null, start: view.start, end: view.end, cursor, selectedId, scaleMode, scarPx: compact ? 17 : 18, strikeAge: (group) => { const at = strikesRef.current.get(group.key); return at == null ? null : now - at; }, pad: { top: compact ? 68 : 72, right: compact ? 78 : 92, bottom: 28, left: compact ? 8 : 20 } });
  };
  ...
  return (
    <section className="rs" data-room={room ?? undefined} data-status={status} aria-label="Replay">
      ...
      <header className="rs-head">
        ...
        {status === "ready" ? <div className="rs-usd-stack"><ReplayPositionCard summary={usdSummary} mark={mark} avgEntryMarketCap={avgEntryMarketCap} buyCoverage={buyCoverage} sellCoverage={sellCoverage}/>{evidenceLine?<span className="rs-evidence-line">{evidenceLine}</span>:null}</div> : null}
      ...
      <nav className="rs-acts" aria-label="Replay actions">
        {room && cohortItems && ticks.length>0 ? <button type="button" className="rs-act rs-act--cohort" aria-pressed={cohortOn} onClick={toggleCohort} title="Other wallets in this room buying this token after this print. Timing only."><Users size={12} /> COHORT {ticks.length}</button> : null}
        <details className="rs-overflow">
          <summary aria-label="Replay options">•••</summary>
          <div className="rs-overflow__menu">
...
function CutPanel({ subject, title, trader, room, bolts, cohort, candles, marketCapPoints, candleCount, start, end, scaleMode, source, evidenceLine, onClose }: { subject: ReplaySubject; title: string; trader: string; room: "fomo" | "afterbell" | null; bolts: TapeBolt[]; cohort: CohortTick[]; candles: TapeCandle[]; marketCapPoints: readonly TapeMarketCapPoint[]; candleCount: number; start: number; end: number; scaleMode: "log" | "linear"; source: string | null; evidenceLine: string|null; onClose: () => void }) {
  ...
  async function render() {
    ...
    try {
      ...
      const cut = await recordReplayCut({ format, title, roomLabel: room ? ROOM_CHIP[room] : "REPLAY", trader, heroGlyph:trader, candles, bolts, cohort, marketCapPoints, evidenceLine, start, end, scaleMode, replayUrl, sourceLine, greyLine: grey ? greyLine : null, onProgress: (value, label) => setProgress({ value, label }) });
