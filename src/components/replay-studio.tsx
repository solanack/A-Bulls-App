...
import { anchorBolts, buildCutManifest, candleSource, candleSourceLabel, cohortTicks, explorerUrl, formatUsdNotional, fullTape, groupBolts, hopSchedule, priceAxisLabel, replayShareUrl, replaySubjectFrom, replayToolInput, STRIKE_MS, tapeCandles, tapeEvents, tapeHeaderLine, tapeScaleFor, tapeUsdSummary, tapeWindow, type CohortTick, type CutFormat, type ReplaySubject, type TapeBolt, type TapeCandle, type TapeUsdSummary } from "@/lib/field/replay-tape";
...
export function ReplayStudio({ muted, onBack, onOpenRoom, watchlist = [], onToggleWatch }: { muted: boolean; onToggleMute?: () => void; onBack: () => void; onOpenRoom: (room: "fomo" | "afterbell") => void; watchlist?: readonly WatchItem[]; onToggleWatch?: (request: ReplayWatchRequest) => void }) {
  ...
  drawRef.current = () => {
    ...
    hitsRef.current = (globalThis as typeof globalThis & { __ABULLS_BOLTS?: BoltHit[] }).__ABULLS_BOLTS = drawTape(ctx, { width: size.w, height: size.h, candles, bolts, cohort: cohortOn ? ticks : undefined, start: view.start, end: view.end, cursor, selectedId, scaleMode, scarPx: compact ? 11 : 14, strikeAge: (group) => { const at = strikesRef.current.get(group.key); return at == null ? null : now - at; }, pad: { top: 44, right: compact ? 52 : 72, bottom: 28, left: compact ? 8 : 20 } });
  };
  ...
  return (
    <section className="rs" data-room={room ?? undefined} data-status={status} aria-label="Replay">
      ...
      <header className="rs-head">
        ...
        {status === "ready" ? <ReplayUsdBox summary={usdSummary} /> : null}
