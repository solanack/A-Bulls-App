...
export type BehaviorFingerprint={observedRoundCount:number;closedRoundCount:number;openRoundCount:number;distinctTokenCount:number;medianHoldMs:number|null;medianObservedBuySol:number|null;meanEntryGapMs:number|null;reentryTokenCount:number;coverage:string;method:string};
export type BehaviorFingerprintResponse={ok:boolean;wallet?:string;fingerprint:BehaviorFingerprint|null;coverage:string;disclosure:string;error?:string};
...
export const getBehaviorFingerprint=createServerFn({method:"GET"}).validator((value:{wallet:string})=>value).handler(async({data}):Promise<BehaviorFingerprintResponse>=>read(`/api/intelligence/research/fingerprint${qs({wallet:data.wallet})}`,{ok:false,coverage:"degraded",fingerprint:null,disclosure:"Observed behavior measurements are unavailable. No behavior was inferred.",error:"fingerprint_unavailable"}));
