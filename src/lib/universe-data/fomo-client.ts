...
export type FomoTrade={signature:string|null;wallet:string;mint:string;chain?:string|null;side:"buy"|"sell";solAmount:number;tokenAmount:number;priceSol:number|null;priceUsd?:number|null;status?:string|null;realizedPnlUsd?:number|null;unrealizedPnlUsd?:number|null;observedAt:number;source:string;sourceKind?:"fomo-reported-position-event"|"a-bulls-observed"};
...
export type FomoClosedTrade={id:string;tradeId:string;handle:string;displayName:string;rank:number|null;chain:string;wallet:string;mint:string;symbol:string|null;pnlUsd:number;entryPriceUsd:number|null;exitPriceUsd:number|null;createdAt:number|null;closedAt:number;fromTs:number;toTs:number;observedIndexed:boolean;chartIndexed:boolean;source:"fomoapi.io"};
export type FomoClosedTradeResultsResponse={ok:boolean;coverage:"partial"|"empty"|"degraded";winners:readonly FomoClosedTrade[];losers:readonly FomoClosedTrade[];replayReadyCount?:number;chartReadyCount?:number;source?:string;disclosure:string;error?:string};
