...
import { composeVolumeSky, countLiveStars } from '../field/volume-sky.ts';
import type { UniverseSnapshot } from '../field/types.ts';

test('indexed token plus actual five-minute market fields becomes a live star without inventing membership',()=>{
  const now=Date.now(), mint='A'.repeat(32);
  const snapshot:UniverseSnapshot={galaxyId:'solana-core',windowStart:now-300000,windowEnd:now,observedEventCount:1,samplingPolicy:'indexed',coverageStatement:'indexed',sources:['index'],particles:[{id:mint,kind:'token',cosmicKind:'star',originGalaxyId:'solana-core',verificationState:'observed',category:'swap',magnitudeBand:0.1,position:[1,2,3],observedAt:now}]};
  ...
  assert.equal(countLiveStars(sky),1);
