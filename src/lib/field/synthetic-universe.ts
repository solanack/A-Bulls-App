...
export function createGalaxySnapshot(galaxy:GalaxyDefinition,count=2800,seed=galaxy.seed):UniverseSnapshot{
  ...
  if(galaxy.id==="fomo"||galaxy.id==="pons")return decorativeGalaxy(galaxy,count,seed);
  const random=mulberry(seed),bounded=Math.max(400,Math.min(16000,Math.trunc(count))),windowStart=1_000_000,durationSeconds=60,flatten=galaxy.id==="pump-fun"?.5:1;
...
export const GALAXY_ZERO_CENTERS:readonly [number,number,number][]=[[-28,18,-10],[28,14,-6],[0,-24,18]];
...
export function createUniverseMapSnapshot(count=2800,seed=861):UniverseSnapshot{
  const random=mulberry(seed),bounded=Math.max(600,Math.min(6000,Math.trunc(count))),definitions=[getGalaxy("fomo"),getGalaxy("pump-fun"),getGalaxy("pons")],centers=GALAXY_ZERO_CENTERS,particles:FieldParticle[]=[],perGalaxy=Math.floor(bounded*.78/definitions.length);
  for(const [galaxyIndex,target] of definitions.entries()){const center=centers[galaxyIndex];for(let index=0;index<perGalaxy;index+=1){const arm=index%3,radius=index===0?0:2.5+Math.pow(random(),.62)*17,angle=radius*.31+arm*Math.PI*2/3+(random()-.5)*.7,isCore=index===0;particles.push({id:`galaxy-map:${target.id}:${index}`,kind:"galaxy-node",cosmicKind:"galaxy",originGalaxyId:"galaxy-zero",verificationState:"directory",observedAt:1_000_000,category:galaxyIndex===0?"swap":galaxyIndex===1?"staking":"program",magnitudeBand:isCore?1:.22+random()*.62,position:[center[0]+Math.cos(angle)*radius,center[1]+(isCore?0:(random()-.5)*4),center[2]+Math.sin(angle)*radius],source:"a-bulls-galaxy-directory",metadata:{targetGalaxyId:target.id,name:target.name,description:target.description,galaxyRole:isCore?"core":"fabric",interactive:true}});}}
  ...
  return{galaxyId:"galaxy-zero",windowStart:1_000_000,windowEnd:1_000_060,observedEventCount:0,samplingPolicy:"galaxy directory; decorative expanse is not chain evidence",coverageStatement:"Galaxy Zero maps Fomo, pump.fun, and PonsFamily. Generic Solana provenance remains available through search, Replay, and Evidence rather than a public galaxy.",sources:["a-bulls-galaxy-directory"],particles};
}
