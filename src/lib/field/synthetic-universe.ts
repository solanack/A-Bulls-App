...
export function createGalaxySnapshot(galaxy:GalaxyDefinition,count=2800,seed=galaxy.seed):UniverseSnapshot{
  ...
  if(galaxy.id==="fomo"||galaxy.id==="pons"||galaxy.id==="pump-fun")return decorativeGalaxy(galaxy,count,seed);
...
export const GALAXY_ZERO_CENTERS:readonly [number,number,number][]=[[0,0,0]];
...
export function createUniverseMapSnapshot(count=2800,seed=861):UniverseSnapshot{
  const random=mulberry(seed),bounded=Math.max(600,Math.min(6000,Math.trunc(count))),definitions=[getGalaxy("fomo")],centers=GALAXY_ZERO_CENTERS,particles:FieldParticle[]=[],perGalaxy=Math.floor(bounded*.78/definitions.length);
  ...
  return{galaxyId:"galaxy-zero",windowStart:1_000_000,windowEnd:1_000_060,observedEventCount:0,samplingPolicy:"galaxy directory; decorative expanse is not chain evidence",coverageStatement:"Galaxy Zero maps Fomo. Generic Solana evidence and retained launch-origin provenance remain available through search, Replay, Evidence, and the Index rather than separate public galaxies.",sources:["a-bulls-galaxy-directory"],particles};
}
