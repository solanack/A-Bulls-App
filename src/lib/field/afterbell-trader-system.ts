...
/** An xStock PLANET at the centre with only the Afterbell trader STARS that retained after-close prints on it. */
export function buildAfterbellPlanetSystemSnapshot(input:{mint:string;symbol:string;name:string|null;stars:readonly FieldParticle[];windowStart:number;windowEnd:number}):UniverseSnapshot{
  const ordered=[...input.stars].sort((a,b)=>Number(a.metadata?.afterbellRank??999)-Number(b.metadata?.afterbellRank??999)).slice(0,24);
  const planet:FieldParticle={id:`afterbell-planet:${input.mint.toLowerCase()}`,kind:"token",cosmicKind:"planet",originGalaxyId:"solana-core",verificationState:"venue-reported",observedAt:input.windowEnd,category:"swap",magnitudeBand:1,position:[0,6,0],source:"DexScreener venue-reported",metadata:{mint:input.mint,symbol:input.symbol,name:input.name,chain:"solana",chainKey:"solana",afterbellEquity:true,systemRole:"afterbell-xstock-planet"}};
  const stars=ordered.map((star,index)=>{const ring=index<8?30:46,count=index<8?Math.min(8,ordered.length):Math.max(1,ordered.length-8),slot=index<8?index:index-8,angle=slot/count*Math.PI*2-Math.PI/2;return{...star,position:[Math.cos(angle)*ring,8+Math.sin(angle*2)*5,Math.sin(angle)*ring] as [number,number,number],metadata:{...(star.metadata??{}),systemRole:"afterbell-xstock-trader-star"}};});
  return{galaxyId:"afterbell",windowStart:input.windowStart,windowEnd:input.windowEnd,observedEventCount:stars.length,samplingPolicy:"xStock PLANET · Afterbell trader STARS with retained after-close prints on this mint · maximum 24",coverageStatement:stars.length?"Only ranked Afterbell wallets with retained after-close prints on this xStock are shown.":"No retained after-close prints exist for this xStock in the current window. The sky stays empty.",sources:["a-bulls-indexed-solana-evidence"],particles:[planet,...stars]};
}
