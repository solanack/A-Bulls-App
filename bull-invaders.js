...
(function (global) {
  ...
  // v7 uses clean, pre-rendered first-party art. The legacy assets remain on
  // disk so the feature-flagged v6 presentation can be restored without a
  // data migration or gameplay rewrite.
  if (V7_FEATURES.chunkyToyArt) BOSSES.forEach(definition => {
    definition.asset = V7_CONFIG.bossAssets?.[definition.key] || definition.asset;
  });
  const SHIPS = Object.freeze([
    ...
    Object.freeze({ id: 4, name: 'Green Runtime', tag: 'Open-circuit hull with neon proof lines.', asset: 'assets/ships/ship-04-green-xray.webp' }),
  ...
  const EPOCH_SECTORS = Object.freeze([
    { name: 'Genesis Grid', tag: 'Green-cyan validator lanes awaken.', colors: ['#14F195', '#00E1FF'] },
    ...
    { name: 'Orca Current', tag: 'Blue formations move like deep water.', colors: ['#3B82F6', '#00E1FF'] },
    { name: 'Jito Relay', tag: 'Staked lances cut the longest lane.', colors: ['#00F5A0', '#14F195'] },
    { name: 'Helius Array', tag: 'Pink telemetry burns across the grid.', colors: ['#FF3B81', '#8B5CF6'] },
    { name: 'Meteora Belt', tag: 'Dual impacts bend the sector path.', colors: ['#E8FF47', '#2DD4BF'] },
    { name: 'Pyth Horizon', tag: 'Seekers track through violet space.', colors: ['#7B61FF', '#00E1FF'] },
    { name: 'Saga Core', tag: 'Purple pulse lanes reach critical speed.', colors: ['#8B5CF6', '#FF3B81'] },
    { name: 'Finality Gate', tag: 'Every route resolves in one last charge.', colors: ['#14F195', '#9945FF'] }
  ]);
  /* POWER_ICON_GUIDE
  ...
  const POWER = {
    rapid: { name: 'Firedancer', asset: 'assets/powerups/ecosystem/rapid.svg', duration: 9000, color: '#00E1FF' },
    ...
    railgun: { name: 'Jito Lance', asset: 'assets/powerups/ecosystem/railgun.svg', duration: 8500, color: '#00F5A0' },
    ...
    bomb2: { name: 'Meteora Dual', asset: 'assets/powerups/ecosystem/bomb2.svg', duration: 0, color: '#E8FF47', instant: true }
  };
  const BEAR_HP_MULTIPLIER = 2;
  const BULL_BOSS_BASE_HP_MULTIPLIER = 2.2;
  // Bullpen NFT-boss damage matrix: default gun stays readable; powered artillery is
  ...
  // EPOCH 1 = existing 19 levels (renamed). EPOCH n≥2 reuse same systems + multipliers + seeded suits.
  ...
  function epochMults() {
    const p = state?.mode === 'daily' ? 1 : currentEpoch();
    return {
      epoch: p,
  ...
  const SOLANA_GRADIENT = Object.freeze(['#14f195', '#00c2ff', '#9945ff']); // green → cyan → purple
  const POWER_COLORS = Object.freeze(Object.fromEntries(Object.entries(POWER).map(([id, definition]) => [id, definition.color])));
  const SOLANA_IMPACT_COLORS = Object.freeze(['#14f195', '#00c2ff', '#9945ff', '#ff3cac', '#ffe45e', '#ff6b35', '#7cff6b']);
  const IMPACT_PALETTES = Object.freeze([
    Object.freeze(['#14f195', '#00c2ff', '#9945ff', '#ff3cac', '#ffe45e']),
    Object.freeze(['#00c2ff', '#7cff6b', '#14f195', '#9945ff', '#ff6b35']),
    Object.freeze(['#ffe45e', '#ff6b35', '#ff3cac', '#9945ff', '#00c2ff']),
    Object.freeze(['#7cff6b', '#14f195', '#00c2ff', '#ff3cac', '#ffe45e'])
  ]);
  ...
  const SPECIAL_ABILITY_CONFIG = Object.freeze([
    Object.freeze({ shipId: 0, theme: 'Solana Lance', color: '#14f195', secondary: '#9945ff', shape: 'lance', trail: 'spray', impact: 'rings' }),
    ...
    Object.freeze({ shipId: 4, theme: 'X-Ray Bolt', color: '#5dff86', secondary: '#00c2ff', shape: 'fork', trail: 'scan', impact: 'scanline' }),
  ...
  const state = {
    ...
    dailyConfig: null, damageTaken: 0, continuesUsed: 0
  };
  ...
  const maxPowerSlots = () => global.Bullion?.maxPowerSlots?.() || 4;
  const arcadeMode = () => state.mode === 'arcade'; // daily uses standard loadout like ranked
  ...
  async function resolveBossRoster() {
    if (!V7_FEATURES.bullpenNftCosmetics) {
      bullBossRoster = [];
      return bullBossRoster;
    }
    bullBossRoster = await global.BullBossRoster?.buildRoster(profile) || [];
    global.BullBossRoster?.preloadRoster?.(bullBossRoster);
    return bullBossRoster;
  }
  ...
  function campaignBullSuitAsset(suit) {
    return suit.asset;
  }
  ...
  function preloadCampaignAssets() {
    ...
    if (V7_FEATURES.bullpenNftCosmetics) {
      (global.BullBossRoster?.suits || []).forEach((suit, index) => {
        const image = bullSuitImages[index] || new Image(); bullSuitImages[index] = image;
        jobs.push(loadImage(image, campaignBullSuitAsset(suit, index, epoch), suit.asset));
      });
    }
    const ship = selectedShip();
  ...
  function preloadAssets() {
    ...
    bossRosterReady = resolveBossRoster();
  ...
  function renderCampaignControls() {
    const rankedMode = global.Bullion?.mode?.() !== 'arcade';
    ...
    const shipTag = $('selectedShipTag'); if (shipTag) shipTag.textContent = selectedShip().tag;
  ...
  async function selectEpoch(value) {
    if (global.Bullion?.mode?.() !== 'arcade') return renderCampaignControls();
  ...
  function reset() {
    ...
    mode: (function(){
      try {
        ...
        if (forced === 'arcade' || global.Bullion?.mode?.() === 'arcade') return 'arcade';
      } catch (_) {}
      return global.Bullion?.mode?.() === 'arcade' ? 'arcade' : 'ranked';
    ...
    if (state.mode === 'daily') {
      try {
        const bossId = sessionStorage.getItem('bbr_daily_boss') || '';
        const modifier = sessionStorage.getItem('bbr_daily_mod') || 'steady';
        const levelIndex = BOSSES.findIndex(boss => boss.id === bossId || boss.key === bossId);
        if (levelIndex < 0 || !['swift', 'dense', 'glass', 'meteor', 'steady'].includes(modifier)) throw new Error('invalid daily selection');
        state.level = levelIndex + 1;
        state.dailyConfig = { bossId, modifier };
      } catch (_) {
        state.mode = 'arcade';
      }
    }
  ...
  function startLevel() {
    ...
    state.wave = 1;
    state.wavesRequired = state.mode === 'daily' ? 1 : state.level;
    state.levelKills = 0;
  ...
  function spawnWave() {
    ...
    const cols = (width < 380 ? 6 : 7) + (state.dailyConfig?.modifier === 'dense' ? 1 : 0);
    ...
    const patternSeed = (state.mode === 'daily' ? 1 : currentEpoch()) * 1009 + state.level * 97 + state.wave * 31;
    ...
    for (let row = 0; row < 4; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        state.enemies.push(logicPools.enemy.acquire({
          ...
          hp: (state.difficulty >= 1.34 && row === 0 ? 2 : 1) * Math.max(BEAR_HP_MULTIPLIER, m.bearHits) * (state.dailyConfig?.modifier === 'glass' ? .6 : 1)
        }));
      }
    }
  ...
  function spawnMemeBoss() {
    ...
    const m = epochMults();
    const hp = Math.max(1, Math.round(144 * state.difficulty * m.memeHp * (state.dailyConfig?.modifier === 'glass' ? .6 : 1)));
  ...
  function spawnBullBoss() {
    ...
    const visual = V7_FEATURES.bullpenNftCosmetics
      ? (bullBossRoster[state.level - 1] || global.BullBossRoster?.visualFor?.({ id: `random-bull-${state.level}`, name: `Random Bull ${state.level}` }, state.level - 1, true))
      : null;
    ...
    const def = V7_FEATURES.bullpenNftCosmetics ? {
      key: `bull-${state.level}`, visual,
      colors: ['#111822', visual?.accent || '#14f195'],
      hands: [[.18, .56], [.82, .56]]
    } : { ...original, finalBoss: true };
  ...
  function shoot(now = performance.now()) {
    ...
    if (powerKey !== 'default') addBurst(center, y + 8, projectileColor(powerKey) || POWER[powerKey]?.color || '#14f195', reducedMotion() ? 2 : 4, 72);
    AudioManager?.sfx.pew();
  }
  function bullpenAimGeometry(boss) {
    const targetX = player.x + player.w / 2, targetY = player.y + player.h * .42;
    return [.29, .71].map((ratio, index) => {
      const shoulderX = boss.x + boss.w * ratio, shoulderY = boss.y + boss.h * .48;
      const angle = Math.atan2(targetY - shoulderY, targetX - shoulderX);
      const reach = boss.w * .27;
      return { index, shoulderX, shoulderY, angle, tipX: shoulderX + Math.cos(angle) * reach, tipY: shoulderY + Math.sin(angle) * reach };
    });
  }
  function enemyShoot(source, speed = 210) {
    const fromBoss = source === state.boss;
    const pattern = fromBoss && source.kind === 'bull' ? source.def.visual?.projectile : 'spread';
    ...
    for (let i = 0; i < count; i++) {
      ...
      const aimedMuzzles = fromBoss && source.kind === 'bull' ? bullpenAimGeometry(source) : null;
      const hand = fromBoss && !aimedMuzzles ? source.def.hands[(source.shotSide + i) % source.def.hands.length] : [.2,.63];
      const aimed = aimedMuzzles?.[(source.shotSide + i) % aimedMuzzles.length];
      const muzzleX = aimed?.tipX ?? source.x + source.w * hand[0], muzzleY = aimed?.tipY ?? source.y + source.h * hand[1];
      ...
      const dailySpeed = state.dailyConfig?.modifier === 'swift' ? 1.35 : state.dailyConfig?.modifier === 'meteor' ? 1.1 : 1;
      const velocity = speed * state.difficulty * dailySpeed;
    ...
    if (fromBoss) { source.shotSide = (source.shotSide + count) % (source.kind === 'bull' ? 2 : source.def.hands.length); source.recoil = 1; }
  }
  function fireEyeDefense(now) {
    ...
    [-.105, .105].forEach(offset => {
      ...
      state.eyeShots.push({ x, y, w: 5, h: 13, vx: Math.cos(angle) * 760, vy: Math.sin(angle) * 760, target, life: .42, angle, color: '#76ff63', player: true });
    });
    addBurst(centerX, player.y + player.h * .39, '#76ff63', 8, 95);
    AudioManager?.sfx.pew?.();
  }
  function updateEyeDefense(dt, now) {
    ...
    state.eyeShots.forEach(shot => {
      ...
      if (hit(shot, target) || Math.hypot(targetX - shot.x, targetY - shot.y) < 13) {
        ...
        addExplosion(targetX, targetY, '#76ff63', .7);
  ...
  function fireRocket(event) {
    ...
    const barrage = arcadeMode() && global.Bullion?.specialWeapon?.();
    ...
    volley.forEach((barrel, index) => {
      ...
      state.specialRockets.push({ x, y, w: 12, h: 28, angle: -Math.PI / 2, vx: 0, vy: -620, speed: 620, target, targetX, targetY, life: 1.7,
        radius: Math.max(92, width * .25) * (barrage ? 1.2 : 1), damage: barrage ? 44 : 28, ability, dead: false, trailClock: 0 });
    });
    updateRocketButton();
    AudioManager?.sfx.pew?.();
  ...
  function updateRocketButton() {
    const button = $('invaderRocketButton'), count = $('invaderRocketCount'), reload = $('invaderRocketReload');
    ...
    if (reload) {
      reload.hidden = !(arcadeMode() && global.Bullion?.storeAvailable?.() && state.running && state.rockets <= 0);
      reload.disabled = state.transitioning;
      const owned = global.Bullion?.entitlement?.('extra_rockets') || 0;
      reload.textContent = owned > 0 ? `USE ROCKET REFILL · ${owned}` : 'GET ROCKET REFILL';
    }
  }
  async function reloadRockets() {
    const button = $('invaderRocketReload');
    if (!arcadeMode() || state.rockets > 0 || state.transitioning) return;
    if (button) button.disabled = true;
    try {
      await global.Bullion?.useConsumable?.('extra_rockets');
      state.rockets = Math.min(8, state.rockets + 3);
      updateRocketButton();
    } catch (error) { toast(error.message || 'Rocket refill unavailable'); }
  ...
  function dropBossMilestoneRewards(boss) {
    ...
    addBurst(center, y + 20, '#c8ff00', 36, 320);
  ...
  function damagePlayer() {
    ...
    if (now < player.invulnerableUntil || state.activePower) {
      addBurst(player.x + player.w / 2, player.y + player.h / 2, state.activePower ? POWER[state.activePower].color : '#c8ff00', 12, 210);
      return;
    }
    state.damageTaken = Math.max(0, Number(state.damageTaken || 0)) + 1;
    state.lives -= state.dailyConfig?.modifier === 'glass' ? 2 : 1;
  ...
  function update(dt, now) {
    ...
    if (state.enemies.length) {
      ...
      if (entering) {
        ...
        const dailyCadence = state.dailyConfig?.modifier === 'meteor' ? 2.85 : 1.65;
        if (Math.random() < dt * dailyCadence * state.difficulty) enemyShoot(state.enemies[Math.floor(Math.random() * state.enemies.length)], 235);
      }
    ...
    if (state.boss) {
      const boss = state.boss, movement = boss.kind === 'bull' ? boss.def.visual?.movement : 'sweep';
      ...
      if (boss.cooldown <= 0) {
        ...
        const dailyCadence = state.dailyConfig?.modifier === 'meteor' ? .68 : 1;
        boss.cooldown = Math.max(.28, (boss.kind === 'bull' ? .95 : 1.16) / state.difficulty * dailyCadence);
      }
    }
    ...
    for (const green of state.playerShots) for (const red of state.enemyShots) {
      if (!green.dead && !red.dead && hit(green, red)) {
        ...
        addExplosion(red.x, red.y, '#c8ff00', .45); AudioManager?.sfx.projectileClash?.();
      }
    }
    for (const shot of state.playerShots) {
      ...
      if (!shot.dead && state.boss && hit(shot, state.boss)) {
        ...
        if (now < (boss.invulnerableUntil || 0) && !shot.phantomMissile) {
          addBurst(shot.x, shot.y, boss.def?.colors?.[1] || '#14f195', 4, 90);
          continue;
        }
    ...
    for (const pickup of state.pickups) if (!pickup.dead && hit(pickup, playerBox)) {
      if (pickup.type === 'life') {
        ...
        pickup.dead = true;
        addBurst(pickup.x + 21, pickup.y + 21, '#5dff9d', 34, 300);
  ...
  function candle(context, shot, isPlayer) {
    ...
    if (!isPlayer) {
      ...
      candleColor = shot.color || '#c8ff00'; strokeHi = '#ffffff'; fillLo = candleColor;
    }
  ...
  function drawBull() {
    ...
    if (state.activePower) {
      ...
      glow.addColorStop(0, 'rgba(200,255,0,.22)'); glow.addColorStop(1, 'rgba(200,255,0,0)');
    ...
    // rectangle, decal, or NFT livery can obstruct the spacecraft. Player size
    ...
    const paidSkin = global.Bullion?.activeSkin?.() || '';
    ...
    formation.forEach(craft => {
      ...
      engine.addColorStop(0, '#ffffff'); engine.addColorStop(.28, '#c8ff00'); engine.addColorStop(1, 'rgba(0,230,118,0)');
      ctx.fillStyle = engine; ctx.shadowColor = '#00e676'; ctx.shadowBlur = 12;
      [-.2, .2].forEach(offset => {
      ...
      ctx.shadowColor = '#00e676'; ctx.shadowBlur = state.shipTier > 1 ? 24 : 14;
      if (baseImage.complete && baseImage.naturalWidth) {
        ...
        ctx.save();
        if (paidSkin === 'ship_skin_solana') ctx.filter = 'hue-rotate(255deg) saturate(1.55) brightness(1.12)';
        else if (paidSkin === 'ship_skin_blackout') ctx.filter = 'saturate(.28) brightness(.7) contrast(1.42)';
  ...
  function drawCaptureFx(now) {
    ...
    ctx.strokeStyle = '#14f195'; ctx.shadowColor = '#00e676'; ctx.shadowBlur = 18;
    ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(0, 0, 18 + progress * 72, 0, Math.PI * 2); ctx.stroke();
    ctx.strokeStyle = '#9945ff'; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(0, 0, 8 + progress * 48, -progress * Math.PI, progress * Math.PI * 1.5); ctx.stroke();
    ctx.restore();
  }
  ...
  function drawBoss(boss) {
    if (boss.kind === 'bull' && V7_FEATURES.bullpenNftCosmetics) return drawBullpenNftBoss(boss);
  ...
  function drawBullpenAimArms(boss, accent) {
    const firingLock = Number(boss.cooldown || 0) < .18;
    const pulse = firingLock ? 1 : reducedMotion() ? .45 : .45 + Math.sin(state.elapsed * 8) * .18;
    bullpenAimGeometry(boss).forEach(arm => {
      const elbowX = arm.shoulderX + Math.cos(arm.angle) * boss.w * .13;
      const elbowY = arm.shoulderY + Math.sin(arm.angle) * boss.w * .13 + boss.h * .035;
      ctx.save();
      ctx.lineCap = 'round'; ctx.lineJoin = 'round';
      ctx.strokeStyle = '#050908'; ctx.lineWidth = Math.max(12, boss.w * .13);
      ctx.beginPath(); ctx.moveTo(arm.shoulderX, arm.shoulderY); ctx.lineTo(elbowX, elbowY); ctx.lineTo(arm.tipX, arm.tipY); ctx.stroke();
      ctx.strokeStyle = accent; ctx.globalAlpha = .68; ctx.lineWidth = Math.max(5, boss.w * .052);
      ctx.shadowColor = accent; ctx.shadowBlur = firingLock ? 18 : 9;
      ctx.beginPath(); ctx.moveTo(arm.shoulderX, arm.shoulderY); ctx.lineTo(elbowX, elbowY); ctx.lineTo(arm.tipX, arm.tipY); ctx.stroke();
      ctx.globalAlpha = 1; ctx.fillStyle = '#090e0c'; ctx.strokeStyle = accent; ctx.lineWidth = 2;
      ctx.beginPath(); ctx.arc(arm.tipX, arm.tipY, Math.max(6, boss.w * .055), 0, Math.PI * 2); ctx.fill(); ctx.stroke();
      ctx.globalAlpha = pulse; ctx.fillStyle = accent; ctx.beginPath(); ctx.arc(arm.tipX, arm.tipY, Math.max(2.5, boss.w * .022), 0, Math.PI * 2); ctx.fill();
      ctx.restore();
    });
  }
  function drawBullpenNftBoss(boss) {
    const visual = boss.def.visual || {}, { w, h } = boss;
    const motion = reducedMotion() ? 0 : 1;
    const hover = Math.sin(state.elapsed * 3 + boss.phase) * 4 * motion, pulse = .5 + Math.sin(state.elapsed * 4.2) * .5 * motion;
    const breathe = 1 + Math.sin(state.elapsed * 4.6 + boss.phase) * .028 * motion;
    const aim = clamp(Number(boss.dir || 0), -1, 1);
    // Bullpen NFT bosses use stable articulated arms aimed at the player. The
    // same computed hand tips are used by enemyShoot(), so muzzle and projectile
    // origins stay visually locked together without changing hitboxes or damage.
    ctx.save(); ctx.translate(boss.x + w / 2, boss.y + h / 2 + hover - boss.recoil * 4); ctx.rotate(Math.sin(state.elapsed * 1.7 + boss.phase) * .035 * motion); ctx.scale(breathe + boss.recoil * .035, 1 / breathe - boss.recoil * .025);
    ctx.save(); ctx.translate(0, h * .09); drawArticulatedBoss(bullSuitImages[visual.suitIndex], w * 1.16, h * .9, boss, true); ctx.restore();

    // The NFT itself supplies the complete Skin, Head and Eye appearance. Its
    // connected edge background was removed during preload; only the exact Bull
    // artwork is composited over the animated suit.
    const cutout = global.BullBossRoster?.cutoutFor?.(visual.mint);
    if (cutout) {
      const sourceHeight = Math.round(cutout.height * .9), headW = w * 1.02, headH = h * .7;
      ctx.save(); ctx.translate(aim * w * .035, -h * .12); ctx.rotate(aim * .075 - boss.recoil * .028); ctx.scale(1 - Math.abs(aim) * .08, 1);
      ctx.shadowColor = visual.accent || '#14f195'; ctx.shadowBlur = 16 + pulse * 9;
      ctx.drawImage(cutout, 0, 0, cutout.width, sourceHeight, -headW / 2, -headH * .69, headW, headH); ctx.restore();
    } else {
      global.BullBossRoster?.loadCutout?.(visual);
    }
    ctx.restore();
    drawBullpenAimArms(boss, visual.accent || '#14f195');
  }
  function drawCanvas() {
    if (!ctx) return;
    ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
    ...
    nebula.addColorStop(0, 'rgba(0,230,118,.13)'); nebula.addColorStop(.42, 'rgba(80,20,125,.08)'); nebula.addColorStop(1, 'rgba(0,0,0,0)');
    ctx.fillStyle = nebula; ctx.fillRect(0, 0, width, height);
    state.stars.forEach(star => {
      ctx.globalAlpha = star.a; ctx.fillStyle = star.s > 1.4 ? '#d8fff0' : '#7affb5'; ctx.shadowColor = '#00e676'; ctx.shadowBlur = star.s > 1.4 ? 6 : 0;
      ctx.beginPath(); ctx.arc(star.x, star.y, star.s, 0, Math.PI * 2); ctx.fill();
    }); ctx.globalAlpha = 1; ctx.shadowBlur = 0;
    ctx.fillStyle = 'rgba(0,230,118,.035)'; ctx.fillRect(0, height * .375, width, height * .625);
    ctx.strokeStyle = 'rgba(0,230,118,.15)'; ctx.setLineDash([5, 8]); ctx.beginPath(); ctx.moveTo(0, height * .375); ctx.lineTo(width, height * .375); ctx.stroke(); ctx.setLineDash([]);
    ctx.strokeStyle = 'rgba(0,230,118,.12)'; ctx.lineWidth = 1;
    ...
    state.pickups.forEach(pickup => {
      const isLife = pickup.type === 'life', def = isLife ? { color: '#5dff9d' } : POWER[pickup.id], image = isLife ? null : powerImages[pickup.id];
    ...
    state.rocketBursts.forEach(burst => {
      const ratio = burst.life / burst.maxLife; const primary = burst.color || '#14f195', secondary = burst.secondary || '#9945ff';
  ...
  function updateHud() {
    ...
    if ($('invaderLevel')) $('invaderLevel').textContent = state.mode === 'daily'
      ? `DAILY · W${state.wave}/${state.wavesRequired}`
      : `${state.level}/${BOSSES.length} · W${state.wave}/${state.wavesRequired}`;
    ...
    if ($('invaderBossName')) $('invaderBossName').textContent = state.boss
      ? (state.mode === 'daily' ? BOSSES[state.level - 1]?.displayName || 'DAILY BOSS' : state.boss.kind === 'bull' ? 'BULLPEN NFT BOSS' : 'ROSTER BOSS')
  ...
  async function start() {
    showView('invadersGame'); resize(); await preloadCampaignAssets(); bullBossRoster = await bossRosterReady;
    if (global.Bullion?.mode?.() === 'arcade') await global.Bullion.refreshBalance({ silent: true });
    ...
    if (state.mode === 'ranked' || state.mode === 'daily') {
      state.paused = true;
  ...
  function recordRun(completedRun) {
    ...
    profile.gameRuns['bull-invaders'] = profile.gameRuns['bull-invaders'].slice(-300); profile.xp = (profile.xp || 0) + Math.floor(completedRun.score / 100); save();
    if (completedRun.mode === 'daily') {
      global.BBRRetentionBridge?.onRunComplete({
        ...completedRun,
        damageTaken: state.damageTaken || 0,
        continuesUsed: state.continuesUsed || 0,
        bossId: sessionStorage.getItem('bbr_daily_boss') || undefined
      })?.catch?.(() => {});
    } else {
      global.BBRRetentionBridge?.onRunComplete(completedRun)?.catch?.(() => {});
    }
    renderAll?.();
  }
  ...
  function renderResultOverlay(won, completedRun) {
    ...
    $('invaderFinalLevel').textContent = state.mode === 'daily' ? 'DAILY' : `${state.level}/${BOSSES.length}`;
    $('shipUnlockStage').hidden = true; $('epochBridgeStage').hidden = true;
    const dailyWon = won && state.mode === 'daily';
    $('invadersOverEyebrow').textContent = dailyWon ? 'DAILY COMPLETE' : won ? 'EPOCH VICTORY' : 'RUN ENDED';
    $('invadersOverTitle').textContent = dailyWon ? 'CHALLENGE LOGGED' : won ? 'EPOCH CONTAINED' : "YOU'RE REKT";
    $('invaderAgain').textContent = dailyWon ? 'RETURN TO HERD' : won ? (V7_FEATURES.themedEpochShips ? 'REVEAL NEW SHIP' : 'SYNC CRAFT') : 'RUN IT BACK';
    $('invaderSecondaryAction').hidden = !won || dailyWon;
    if (won) $('invaderSecondaryAction').textContent = state.nextEpoch ? `SKIP TO EPOCH ${state.nextEpoch}` : 'RETURN TO HERD';
  ...
  async function autoStartNextEpoch() {
    ...
    if (!next) return returnToHerd();
    state.flowToken += 1; setFlow(FLOW_STATES.AUTO_START);
    if (global.Bullion?.mode?.() !== 'arcade') global.Bullion?.setMode?.('arcade');
  ...
  function advanceVictoryFlow() {
    ...
    if (state.flow === FLOW_STATES.SHIP_UNLOCK) {
      if (!reward.nextEpoch) {
        ...
        $('invadersOverTitle').textContent = 'ALL EPOCHS CONTAINED'; $('invaderAgain').textContent = 'RETURN TO HERD';
    ...
    else if (state.flow === FLOW_STATES.COMPLETE) returnToHerd();
  }
  async function returnToHerd() {
    ...
    showView('addBulls', { fromHistory: true });
  }
  function end(won) {
    ...
    state.resultWon = won;
    const epoch = currentEpoch();
    const reward = won && state.mode !== 'daily' ? grantEpochClear(epoch) : null;
    const completedRun = { ts: Date.now(), score: state.score, level: state.level, won, mode: state.mode, kills: state.kills, bossesDefeated: state.bossesDefeated, elapsedMs: Math.max(1000, Math.round(performance.now() - runStartedAt)), epoch, damageTaken: state.damageTaken, continuesUsed: state.continuesUsed, bossId: state.dailyConfig?.bossId, modifier: state.dailyConfig?.modifier };
    if (state.recordingRequested) {
    ...
    if (won && state.mode === 'daily') { setFlow(FLOW_STATES.COMPLETE); AudioManager?.sfx.success?.(); }
    else if (won) beginVictoryFlow(reward);
    else { setFlow(FLOW_STATES.DEATH); AudioManager?.sfx.gameOver?.(); }
  }
  async function handlePrimaryResult() {
    if (state.mode === 'daily' && state.flow === FLOW_STATES.COMPLETE) return returnToHerd();
  ...
  async function handleSecondaryResult() {
    ...
    if (state.resultWon) return returnToHerd();
  }
  ...
  function init() {
    ...
    $('invaderRocketReload')?.addEventListener('click', reloadRockets);
    ...
    window.addEventListener('bbrs:boss-roster-change', () => { bossRosterReady = resolveBossRoster(); });
