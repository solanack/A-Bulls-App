/* Shared media background manager for the menu and Bull Invaders. */
(function (global) {
  'use strict';
  ...
  function syncLandingPresentation(profile) {
    ...
    const hub = home?.querySelector?.('.home-community-hub');
    const bar = document.getElementById('landingTwitchBar');
    const label = document.getElementById('landingTwitchLabel');
    ...
    const mediaActive = twitchActive || youtubeActive;
    home?.classList?.toggle('twitch-media-mode', twitchActive);
    home?.classList?.toggle('youtube-media-mode', youtubeActive);
    if (bar) bar.hidden = !twitchActive;
    if (hub) {
      hub.inert = mediaActive;
      if (mediaActive) hub.setAttribute('aria-hidden', 'true');
      else hub.removeAttribute('aria-hidden');
    }
    if (label) label.textContent = twitchActive
      ? (twitch.type === 'channel' ? `@${String(twitch.value).toUpperCase()}` : `VOD ${String(twitch.value).replace(/^v/i, '')}`)
      : 'COMMUNITY STREAM';
    return mediaActive;
  }
  ...
  const Manager = {
    ...
    async applyMenu(profile) {
      ...
      // v7 defaults to the first sculpted sector. The legacy landing artwork
      // remains deployable behind the art flag; uploads still render above it.
      fallback.src = global.BBRV7?.features?.chunkyToyArt
        ? (global.BBRV7.config.levelBackgrounds?.[0] || 'assets/v7/backgrounds/level-01.webp')
        : 'assets/opening/menu-bull-solana.webp';
    ...
    async applyGame(profile, target = currentTarget()) {
      ...
      // steals audio inside Bull Invaders.
    ...
    landingPresentationSnapshot() {
      ...
      return {
        ...
        hubHidden: hub?.getAttribute?.('aria-hidden') === 'true' && hub?.inert === true,
