...
Any trader opens the same sheet: REPLAY · WATCH TRADER · EVIDENCE. Replay is a full-frame tape of every retained candle for the token (or an event tape marked "Candles unavailable"). EVM tokens without a chosen quote are priced in USD. Each of the trader's buys (lime) and sells (rose) strikes the candle once as lightning, then stays as a small scar. COHORT adds muted ticks for other wallets in the same room buying the token after the trader's first print. These ticks are timing only. Tapping a scar or tick opens its evidence. SHARE copies a URL that reopens the exact Replay, and CUT renders a 1080×1920 or 1920×1080 video that ends on VERIFY plus that URL, with a manifest of wallet, mint, window, signatures, and candle source.

THAT DAY lists public X posts that mention the token (ticker, name, or mint) on the UTC date of the first print, then +1, +3 and +7. Opening it, or opening Replay, asks the Intelligence Worker to fetch once per mint and date and retain matches in `social_posts_retained`. The Worker uses official X search when `X_BEARER_TOKEN` is set on it. Otherwise it uses the Exa route that [Agent-Reach](https://github.com/solanack/Agent-Reach) uses, which needs no credentials. `.github/workflows/social-reach.yml` runs `scripts/social-reach/runner.py` with the Agent-Reach fork on a host. It needs `SOCIAL_INGEST_TOKEN` as both a repo secret and a Worker secret. It can optionally use `TWITTER_AUTH_TOKEN`/`TWITTER_CT0` from a dedicated account, never the owner's main account. None of these reach the browser. An empty window says so, and a post is never presented as the reason for a print.
