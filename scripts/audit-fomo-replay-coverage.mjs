...
const rows=[...pairs.values()].sort((a,b)=>a.rank-b.rank||a.handle.localeCompare(b.handle)||a.chain.localeCompare(b.chain)||a.token.localeCompare(b.token)).map(pair=>{
  ...
  const candleCount=n(candles?.count),quotes=[...(candles?.quotes||[])],marketCount=marketCounts.get(tokenKey(pair.chain,pair.token))||0,replayEvidence=pair.timedTradeRows>0||evt.count>0,chartEvidence=candleCount>0||pair.providerPricePoints>0,missingWallet=!wallet,unknownChain=pair.chain==="unknown-evm"||pair.chain==="unknown";
