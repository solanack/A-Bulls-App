# A Bulls App Worker 5.5.1

This Worker matches Pages 7.3.1. It preserves the existing package identity (`com.abullsapp.app`) and the read-only wallet boundary: there are no wallet connections, signatures, transactions, custody flows, or client-side provider secrets.
...
3. Keep the Workers AI binding named `AI` for LIFE. LIFE sends only a minimized public-history pattern summary to the model and writes no LIFE record to D1.
...
7. Deploy and confirm `/api/health` reports Worker `5.5.1`, retention/leaderboard availability that matches the bindings, and every commerce service as `false`.
8. Deploy the matching Pages 7.3.1 package.
