...
(function (global) {
  ...
  function prewarm() {
    ...
    if (!noiseBuffer || noiseBuffer.sampleRate !== c.sampleRate) {
      ...
      // Deterministic-enough white noise buffer created once, not during an explosion.
  ...
  function startMusic() {
    ...
    let step = 0;
    const scales = [
      [55, 82.5, 110], [49, 73.5, 98], [61.7, 92.5, 123.5], [58.3, 87.3, 116.5],
      [65.4, 98, 130.8], [51.9, 77.8, 103.8], [69.3, 103.8, 138.6], [46.2, 69.3, 92.5],
      [73.4, 110, 146.8], [41.2, 61.7, 82.4]
    ];
    const tick = () => {
      if (!settings.musicOn || !unlocked) return;
      const epoch = Math.max(1, Math.min(10, Number(global.profile?.selectedEpoch) || 1));
      const notes = scales[epoch - 1];
      const root = notes[step % notes.length];
      beep(root, .16, 'sawtooth', .032, musicGain);
      if (step % 2 === 0) beep(notes[(step + 1) % notes.length] * 2, .09, 'triangle', .022, musicGain);
      step = (step + 1) % 12;
      musicTimer = setTimeout(tick, 440 + (epoch % 3) * 20);
    };
    tick();
  }
  function stopMusic() {
