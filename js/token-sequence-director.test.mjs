...
const bundle={storyType:'token-sequence',claims:[
  {id:'sequence-first-event'},{id:'sequence-phase-1'},{id:'sequence-largest-token-delta'},{id:'sequence-direction-mix'}
...
test('scene plan includes only reconstruction beats with supported claims',()=>{
  ...
  assert.deepEqual(plan.map(scene=>scene.type),['sequence-opening','market-phase','largest-observed-trade','direction-mix','evidence-summary']);
