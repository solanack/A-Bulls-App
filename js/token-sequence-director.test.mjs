...
const bundle={storyType:'token-sequence',claims:[
  {id:'sequence-first-event'},{id:'sequence-largest-token-delta'},{id:'sequence-direction-mix'}
...
test('scene plan includes only reconstruction beats with supported claims',()=>{
  ...
  assert.deepEqual(plan.map(scene=>scene.type),['sequence-opening','largest-observed-trade','direction-mix','evidence-summary']);
  assert.equal(plan.some(scene=>scene.type==='selected-price-movement'),false);
});

test('direction summary preserves ordered reconstruction and disclosure',()=>{
