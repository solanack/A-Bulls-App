...
async function setup(){
  ...
  let app=null,universe=null;
  const host=node('div');host.id='nextProductShell';document.body.append(host);
  const instances=new Map(),adapters=new ProductAdapterRegistry();
  ...
  function intelligenceProduct(context={}){
    const mountNode=node('div','intelligence-product-vnext'),switcher=node('div','intelligence-mode-switcher'),surface=node('div','intelligence-mode-surface');
    let active=null;
    const walletButton=node('button','secondary intelligence-mode-switcher__button','WALLET / TRANSACTION'),marketButton=node('button','secondary intelligence-mode-switcher__button','TOKEN MARKET');
    walletButton.type=marketButton.type='button';switcher.append(walletButton,marketButton);mountNode.append(switcher,surface);
    const setActive=mode=>{walletButton.classList.toggle('active',mode==='wallet');marketButton.classList.toggle('active',mode==='market');};
    ...
    const showMarket=()=>{const prefill=context.request?.token||context.request?.mint||context.request?.entityId||context.request?.query||'';destroyActive();setActive('market');active=new TokenMarketWorkspace({host:surface,apiBase:globalThis.BBRConfig?.apiBase||location.origin,prefillMint:prefill,onCreateStory:openStory});};
    const kind=String(context.request?.entityKind||context.request?.kind||'').toLowerCase(),destination=String(context.request?.destination||'').toLowerCase(),startsInMarket=kind==='token'||kind==='token-mint'||destination==='market-sequence'||destination==='sequences';
    walletButton.addEventListener('click',showWallet);marketButton.addEventListener('click',showMarket);startsInMarket?showMarket():showWallet();
  ...
  adapters.register('trickster',{activate(){
    if(!tricksterRequested){const n=node('section','product-home');n.append(node('h1','','Creator unavailable'),node('p','','The evidence-backed creator remains fail-closed until its validation/export path is enabled.'));return n;}
    tricksterFieldCleanup?.();tricksterFieldCleanup=null;instances.get('trickster')?.destroy();
    const studioHost=node('div'),studio=new TricksterStudio({
      host:studioHost,
      onOpenEvidence:()=>{const content=adapters.activate('intelligence',{source:'trickster'});mount(content,'EVIDENCE','evidence');},
      onShare:async manifest=>{const apiBase=globalThis.BBRConfig?.apiBase||location.origin;const published=await publishStoryManifest(manifest,{apiBase});const url=storyShareUrl(published.shareId,{origin:location.origin});return Object.freeze({...published,url});},
      onExport:async detail=>{const validation=await validateStoryForExport(detail.manifest,{apiBase:globalThis.BBRConfig?.apiBase||location.origin});let rendering=null;const narration=validation.validated===true?buildTricksterNarrationPlan(validation.manifest,detail.timeline):null;const progress={onProgress:item=>globalThis.dispatchEvent(new CustomEvent('abulls:trickster-render-progress',{detail:item}))};if(validation.validated===true&&detail.renderPlan?.length){if(detail.manifest?.storyType==='transaction-replay'||detail.manifest?.storyType==='token-sequence')rendering=await renderEventStoryVideo({...detail,manifest:validation.manifest},progress);else if(detail.manifest?.storyType==='wallet-comparison')rendering=await renderWalletComparisonVideo({...detail,manifest:validation.manifest},progress);}const result=rendering?Object.freeze({...validation,...rendering,manifest:validation.manifest,narration}):Object.freeze({...validation,narration});globalThis.dispatchEvent(new CustomEvent('abulls:trickster-export',{detail:{...detail,manifest:validation.manifest,validation,narration,rendering,result}}));return result;}
    });
    instances.set('trickster',studio);instances.set('tricksterHost',studioHost);if(globalThis.BBR_TRICKSTER_EVIDENCE){studio.loadEvidence(globalThis.BBR_TRICKSTER_EVIDENCE);bindTricksterField(studio);}return studioHost;
  },deactivate(){tricksterFieldCleanup?.();tricksterFieldCleanup=null;instances.get('trickster')?.destroy();instances.delete('trickster');instances.delete('tricksterHost');}});
  ...
  if(!app.mounted){host.remove();return;}
  legacyApp?.remove();

  if(universeRequested){
    universe=new UniverseExperience({host:app.shell.fieldHost,apiBase:globalThis.BBRConfig?.apiBase||location.origin,THREE,onDestinationRequest:async(destination,entity)=>{globalThis.dispatchEvent(new CustomEvent('abulls:universe-selection',{detail:{destination,entity}}));const content=adapters.activate('intelligence',{source:'field-selection',request:{...destination,query:destination.entityId}});mount(content,'INTELLIGENCE','intelligence');}});
    universe.start();
  }else{
    const fallback=node('div','universe-shell');fallback.append(node('div','universe-fallback','LIVE FIELD DATA IS DISABLED · SEARCH AND READ-ONLY INTELLIGENCE REMAIN AVAILABLE'));app.shell.fieldHost.replaceChildren(fallback);
  }
  ...
  globalThis.BBRNextExperience=Object.freeze({...app,loadStory:openStory,destroy(){tricksterFieldCleanup?.();tricksterFieldCleanup=null;universe?.destroy();app?.destroy?.();}});
}
