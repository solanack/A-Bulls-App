...
import { IsolatedQuantumOrganismController } from './quantum/isolated-organism-controller.mjs';
...
async function setup(){
 ...
 let app=null,universe=null,queryExperience=null,queryOrganism=null;const host=node('div');host.id='nextProductShell';document.body.append(host);const instances=new Map(),adapters=new ProductAdapterRegistry();let gameHost=null,gameInitialized=false,removeGameBridge=null,tricksterFieldCleanup=null;
 ...
 const setQueryVisual=(active)=>{
   queryOrganism?.setActive?.(Boolean(active));
   if(app?.shell?.fieldHost) app.shell.fieldHost.classList.toggle('query-organism-active',Boolean(active));
 };
 globalThis.addEventListener('abulls:query-state',(event)=>setQueryVisual(Boolean(event?.detail?.active)));
 ...
 globalThis.addEventListener('abulls:query-return',()=>{
   setQueryVisual(false);
   app?.shell?.closeWorkspace?.();
 });
 if(universeRequested){
   universe=new UniverseExperience({host:app.shell.fieldHost,apiBase:globalThis.BBRConfig?.apiBase||location.origin,THREE,onDestinationRequest:async(destination,entity)=>{globalThis.dispatchEvent(new CustomEvent('abulls:universe-selection',{detail:{destination,entity}}));}});universe.start();
   const attachOrganism=(gpu)=>{if(!gpu||queryOrganism)return;queryOrganism=new IsolatedQuantumOrganismController({host:app.shell.fieldHost,THREE:gpu});setQueryVisual(Boolean(globalThis.__ABULLS_QUERY_ACTIVE));};
   if(THREE) attachOrganism(THREE);
   loadedThreePromise.then((loadedThree)=>{const gpu=loadedThree&&typeof loadedThree.WebGLRenderer==='function'?loadedThree:null;if(!gpu)return;globalThis.THREE=gpu;universe.promoteRenderer?.(gpu);attachOrganism(gpu);});
 }else{const fallback=node('div','universe-shell');fallback.append(node('p','field-disabled','Live field rendering is paused. Search remains available.'));app.shell.fieldHost.replaceChildren(fallback);}
 ...
 globalThis.BBRNextExperience=Object.freeze({...app,loadStory:openStory,destroy(){tricksterFieldCleanup?.();tricksterFieldCleanup=null;queryOrganism?.destroy?.();queryExperience?.destroy();universe?.destroy();app?.destroy?.();}});
}
