...
async function setup() {
  ...
  function intelligenceProduct(context={}){
    ...
    const requestKind=String(context.request?.entityKind||context.request?.kind||'').toLowerCase(),destination=String(context.request?.destination||'').toLowerCase(),startsInMarket=requestKind==='token'||requestKind==='token-mint'||destination==='market-sequence';
    walletButton.addEventListener('click',showWallet);marketButton.addEventListener('click',showMarket);if(startsInMarket)showMarket();else showWallet();
