...
(function (global) {
  ...
  async function hashRun(payload) {
    const canonical = ['abulls-v8.0.1', payload.mode, payload.game, payload.score, payload.elapsedMs, payload.kills || 0, payload.bossesDefeated || 0, payload.nonce, payload.challengeId].join('|');
    return hex(await crypto.subtle.digest('SHA-256', new TextEncoder().encode(canonical)));
  }
  async function beginRun(mode, game = 'game') {
    if (mode !== 'ranked') return null;
    challenges.delete(mode);
    const challenge = await global.BBRSApi.authenticatedPost('/api/leaderboard/challenge', { game, mode }, { timeoutMs: 2500 });
  ...
  async function submit(game, run) {
    try {
      ...
      return await global.BBRSApi.authenticatedPost('/api/leaderboard/submit', payload);
    } catch (_) { return null; }
  }
