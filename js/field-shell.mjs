...
export function createFieldShell({host,serviceState='ready',onCommand,onSearch}){
  ...
  const investigationDock=el('aside','field-shell__investigation');investigationDock.hidden=true;investigationDock.setAttribute('aria-live','polite');
  ...
  const hint=el('div','field-shell__hint',MODE_HINTS.explore);chrome.append(top,investigationDock,commandDock,hint);root.append(fieldHost,chrome,workspace);host.replaceChildren(root);
  ...
  function renderInvestigation(hub){investigationDock.replaceChildren();if(!hub?.focusId){investigationDock.hidden=true;return;}investigationDock.hidden=false;const heading=el('div','field-shell__investigation-title',`${String(hub.focusKind||'entity').toUpperCase()} FOCUS`),count=el('div','field-shell__investigation-count',hub.evidenceCount?`${hub.evidenceCount} EVIDENCE-BACKED CONNECTION${hub.evidenceCount===1?'':'S'}`:'NO INDEXED CONNECTION EVIDENCE');const actions=el('div','field-shell__investigation-actions');for(const action of (Array.isArray(hub.actions)?hub.actions:[])){const button=el('button','field-shell__investigation-action',action.label);button.type='button';button.dataset.simulation=String(Boolean(action.simulation));button.addEventListener('click',()=>onCommand?.(action.id,{investigationHub:hub,focusId:hub.focusId,simulation:Boolean(action.simulation)}));actions.append(button);}investigationDock.append(heading,count,actions);investigationDock.title=hub.disclosure||'';}
