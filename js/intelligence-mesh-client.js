...
(function (global) {
  ...
  async function post(path, payload) {
    return requireApi().envelope(path, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(payload || {}) });
  }
  ...
  async function verification(wallet, signature) {
    const w=needWallet(wallet); if(!s(signature)) throw new Error('Wallet and signature are required.');
    return post('/api/intelligence/verification', { wallet:w, signature:s(signature) });
  }
  ...
  function coverageLabel(row) {
    ...
    return Number(row.complete_to_genesis) === 1 ? 'COMPLETE HISTORY' : 'PARTIAL HISTORY';
  }
  ...
  global.IntelligenceMesh = Object.freeze({
    validWallet,status,sourceHealth,chainRadar,chainWeather,coverage,walletDna,timeMachine,
    constellation,museum,chainLens,candles,ghostPortfolio,parallelUniverse,walletRivalry,
