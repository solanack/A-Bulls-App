export const LIVING_QUANTUM_ORGANISM = Object.freeze({
  orientationY: 0,
  layers: Object.freeze({ dermal: 0.6, subdermal: 0.25, core: 0.15 }),
  pointBudget: Object.freeze({ low: 48000, normal: 72000, high: 96000 }),
...
export function isFacialVoid(x, y, z) {
  ...
  const nx = x / 39;
  const ny = y / 50;
  if (-0.18 < ny && ny < -0.03 && 0.025 < Math.abs(nx) && Math.abs(nx) < 0.1)
    return true;
...
export function lightingForNormal(nx, ny, nz, depth = 0) {
  ...
  let light =
    cfg.ambient +
    dot(cfg.key) * cfg.keyStrength +
    dot(cfg.fill) * cfg.fillStrength +
    Math.pow(dot(cfg.rim), 3) * cfg.rimStrength;
