...
function capabilitiesFromBrowser() {
  ...
  let webgl2 = false;
  let webgl = false;
  try { webgl2 = Boolean(webgl2Canvas.getContext('webgl2')); } catch { webgl2 = false; }
  try { webgl = Boolean(webglCanvas.getContext('webgl') || webglCanvas.getContext('experimental-webgl')); } catch { webgl = false; }
  return {
    webgl2,
    webgl: webgl || webgl2,
...
function cssRgb([r, g, b], alpha = 1) {
  return `rgba(${Math.round(r * 255)},${Math.round(g * 255)},${Math.round(b * 255)},${alpha})`;
}

export class UniverseRenderer {
  #host; #snapshot; #three; #renderer; #scene; #camera; #points; #stars; #material; #starMaterial;
  #raf = 0; #lastFrame = 0; #quality; #transition = new HyperspaceTransition(); #onSelect;
  #destroyed = false; #destinationReady = false; #mode = 'explore';
  #modeTarget = [1, 1, 0]; #modeCurrent = [1, 1, 0];
  #onMode; #onReplay; #onTemporalReplay; #onMarketPhase; #onStoryScene;
  #replayEffects = []; #phaseEffect = null; #phaseUntil = 0; #storyField = null;
  #cameraTarget = { distance: 125, yaw: 0.4, pitch: 0.18, target: [0, 0, 0] };
  #investigationHub = null; #hubObjects = [];
  #backend = 'none'; #canvas = null; #ctx = null; #visible = [];
  #starfield = createStarfield(); #pointer = { active: false, x: 0, y: 0, moved: false };
  ...
  constructor({ host, snapshot, THREE, onSelect, capabilities = capabilitiesFromBrowser() }) {
    ...
    this.#onTemporalReplay = (e) => this.#queueReplayEffects(fieldReplayBurst(e?.detail?.events || []));
    ...
    globalThis.addEventListener('abulls:temporal-replay-events', this.#onTemporalReplay);
    globalThis.addEventListener('abulls:field-market-phase', this.#onMarketPhase);
  ...
  #mountThree() {
    ...
    this.#observeHost();
    canvas.addEventListener('webglcontextlost', this.#onContextLost, { passive: false });
    this.#resize();
  ...
  #mountCanvas() {
    ...
    this.#backend = 'canvas';
    this.#visible = this.#snapshot.particles.slice(0, this.#quality.profile.particleLimit);
    this.#bindCanvasInput(canvas);
    this.#observeHost();
    this.#resize();
    this.#lastFrame = performance.now();
  ...
  #geometryForSnapshot(snapshot) {
    ...
    visible.forEach((entity, index) => {
      ...
      sizes[index] = 1.6 + entity.magnitudeBand * 4.6;
    });
  ...
  #queueReplayEffects(effects) {
    ...
    if (this.#backend === 'webgl' && this.#scene) {
      ...
      for (const effect of effects.slice(0, 32)) {
        ...
        const [ax, ay, az] = resolved.position;
        const geometry = new T.BufferGeometry();
        let object;
        if (effect.kind === 'lightning') {
          const vertices = [];
          for (let i = 0; i < 5; i++) {
            const jitter = (i === 0 || i === 4) ? 0 : (i % 2 === 0 ? 1 : -1) * (2 + effect.intensity * 4);
  ...
  #animateReplayEffects(now) {
    for (let i = this.#replayEffects.length - 1; i >= 0; i--) {
      const object = this.#replayEffects[i];
      const age = now - object.userData.born;
      const progress = age / object.userData.life;
      if (progress >= 1) {
      ...
      if (this.#backend === 'webgl' && object.material) {
        object.material.opacity = (object.userData.simulation ? 0.32 : 0.95) * (1 - progress);
        object.scale.setScalar(1 + progress * (object.isPoints ? 2.5 : 0.35));
        if (object.userData.simulation) object.rotation.z = progress * 0.16;
      }
    }
  }

  #clearHubObjects() {
    for (const object of this.#hubObjects) {
      this.#scene?.remove(object);
      object.geometry?.dispose();
      object.material?.dispose();
    }
  ...
  setInvestigationHub(hub) {
    ...
    const entities = this.#points?.geometry?.userData?.entities || this.#visible || [];
    const byId = new Map(entities.map((entity) => [String(entity.id), entity]));
    const focus = byId.get(String(hub.focusId));
    if (focus) this.focusEntity(focus);
    if (this.#backend !== 'webgl' || !this.#scene) return hub;
    const T = this.#three;
    for (const edge of (Array.isArray(hub.edges) ? hub.edges : []).slice(0, 160)) {
  ...
  setMode(mode = 'explore') {
    ...
    if (this.#mode !== 'trickster') {
      ...
      if (this.#mode === 'explore' && !this.#investigationHub) this.clearFocus();
    }
    return this.#mode;
  }
  ...
  updateSnapshot(snapshot) {
    ...
    this.#snapshot = snapshot;
    this.#visible = snapshot.particles.slice(0, this.#quality.profile.particleLimit);
  ...
  #drawCanvas(now, warp) {
    ...
    const nebula = ctx.createRadialGradient(width * 0.5, height * 0.42, 20, width * 0.5, height * 0.5, Math.max(width, height) * 0.72);
    nebula.addColorStop(0, 'rgba(28, 32, 48, 0.55)');
    nebula.addColorStop(0.45, 'rgba(10, 12, 18, 0.2)');
    ...
    for (let entityIndex = 0; entityIndex < this.#visible.length; entityIndex += 1) {
      ...
      const renderPosition = this.#canvasQueryPosition(entity, entityIndex, now);
      const projected = this.#projectPoint(renderPosition, width, height);
      if (projected.depth < 6) continue;
      const warped = projected.x + warp * (renderPosition[0] * 0.4);
      const size = Math.max(1.2, (1.4 + entity.magnitudeBand * 4.2) * (90 / projected.depth) * this.#modeCurrent[1]);
      const color = COLORS[entity.category] ?? COLORS.unknown;
      const alpha = Math.max(0.18, Math.min(0.95, (22 / projected.depth) * intensity));
      const glow = ctx.createRadialGradient(warped, projected.y, 0, warped, projected.y, size * 3.2);
  ...
  #frame = (now) => {
    ...
    if (this.#backend === 'webgl') {
      ...
      const desiredY = ty + Math.sin(camera.pitch) * camera.distance;
      const desiredZ = tz + Math.cos(camera.yaw) * camera.distance;
      const follow = this.#pointer.active ? 0.22 : 0.16;
  ...
  destroy() {
    ...
    globalThis.removeEventListener('abulls:field-replay-events', this.#onReplay);
    globalThis.removeEventListener('abulls:temporal-replay-events', this.#onTemporalReplay);
    globalThis.removeEventListener('abulls:field-market-phase', this.#onMarketPhase);
    globalThis.removeEventListener('abulls:field-story-scene', this.#onStoryScene);
    this.#teardownGpu();
  }
}
