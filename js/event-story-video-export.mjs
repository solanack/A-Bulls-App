...
const RENDERABLE_STORY_TYPES=new Set(['transaction-replay','token-sequence']);
...
export async function renderEventStoryVideo(detail={}, {
  ...
  if(!RENDERABLE_STORY_TYPES.has(manifest.storyType)||!Array.isArray(renderPlan)||!renderPlan.length)return fallback('evidence_story_render_plan_unavailable');
  ...
  const bundle={storyType:manifest.storyType,replayEvents:detail.replayEvents||[],candles:detail.candles||[],marketContext:detail.marketContext||null,marketReplay:detail.marketReplay||null,priceSelection:detail.priceSelection||null};
