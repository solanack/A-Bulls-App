...
export function mountMarketContextReplay({context,request}={}){
  ...
  const price=node('select','intelligence-market-replay-price');price.setAttribute('aria-label','Market replay price overlay');const options=marketReplayPriceOptions(context);for(const item of options){const option=document.createElement('option');option.value=item.id;option.textContent=item.label;price.append(option);}
  ...
  const disclosure=node('p','intelligence-context-disclosure','Time-only is the default. Price is overlaid only after you explicitly select an indexed quote pair. Surrounding wallet events are contextual observations only and do not imply coordination, common ownership, strategy, intent, or causation.');
  tools.append(copy,price,play,host,disclosure);
  ...
  const render=()=>{
    ...
    const option=options.find(item=>item.id===price.value)||options[0];
  ...
  price.addEventListener('change',()=>{if(activePlayer&&activeHost===host)render();});
  return true;
}
