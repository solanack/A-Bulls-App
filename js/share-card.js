...
(function (global) {
  ...
  async function exportCard(summary) {
    ...
    const background = context.createLinearGradient(0, 0, 1200, 675); background.addColorStop(0, '#171820'); background.addColorStop(.58, '#292a34'); background.addColorStop(1, '#272331');
    context.fillStyle = background; context.fillRect(0, 0, 1200, 675);
    context.strokeStyle = 'rgba(176,211,196,.16)'; context.lineWidth = 2;
    for (let x = -100; x < 1300; x += 70) { context.beginPath(); context.moveTo(600 + (x - 600) * .25, 300); context.lineTo(x, 675); context.stroke(); }
    context.fillStyle = '#c4afcf'; context.font = '700 34px Manrope, system-ui, sans-serif'; context.fillText('A BULLS APP', 72, 86);
    ...
    stats.slice(0, 3).forEach((entry, index) => {
      ...
      context.strokeStyle = 'rgba(176,211,196,.42)'; context.strokeRect(x, 245, 315, 190);
      ...
      context.fillStyle = index === 0 ? '#c4afcf' : '#b0d3c4'; context.font = '700 62px Sora, system-ui, sans-serif'; context.fillText(String(entry.value), x + 28, 385);
    });
    ...
    context.fillStyle = '#a8c9d5'; context.fillRect(72, 610, 1056, 5);
