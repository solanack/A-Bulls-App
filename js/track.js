...
(function (global) {
  ...
  const WALLET_SESSION_CACHE_PREFIX = 'bbrs_wallet_analysis_v3:';
  const WALLET_SESSION_MAX_AGE = 15 * 60_000;
  const WALLET_REFRESH_PREFIX = 'bbrs_wallet_refreshed_v3:';
  const state = {
    address: null,
    range: '24h',
    requestId: 0,
  ...
  const walletCacheKey = (address, range = state.range) => WALLET_SESSION_CACHE_PREFIX + String(address || '').trim() + ':' + String(range || '24h');
  function readWalletSession(address) {
    try {
      const cached = JSON.parse(sessionStorage.getItem(walletCacheKey(address, state.range)) || 'null');
  ...
  function writeWalletSession(address) {
    ...
    try { sessionStorage.setItem(walletCacheKey(address, state.range), JSON.stringify({ address, range: state.range, overview: state.overview, activity: state.activity, savedAt: Date.now() })); } catch (_) {}
  }
  function removeWalletSession(address) {
    try {
      if (address) {
        Object.keys(RANGE_LABELS).forEach(range => sessionStorage.removeItem(walletCacheKey(address, range)));
        sessionStorage.removeItem(WALLET_REFRESH_PREFIX + address);
      }
    } catch (_) {}
  }

  function setWalletLoading(active, message = '') {
    const root = el('walletAnalyticsPanel');
    const loading = el('trackLoading');
    if (root) root.classList.toggle('is-loading', active === true);
    if (loading) loading.hidden = active !== true;
    if (message) setText('trackPhase', message);
  }
  ...
  function renderTimeline(data) {
    const root = el('trackTimeline');
    if (!root) return;
    const buckets = Array.isArray(data.dayBuckets) ? data.dayBuckets.slice(-30) : [];
    if (!buckets.length) {
      root.innerHTML = '<p class="notice">No daily activity buckets in this range.</p>';
      return;
    }
    const max = Math.max(1, ...buckets.map(item => Number(item.count || 0)));
    root.innerHTML = `<div class="wallet-timeline-bars" role="img" aria-label="Transactions by active day">${buckets.map(item => {
      const count = Number(item.count || 0);
      const height = Math.max(5, Math.round(count / max * 100));
      return `<i style="--bar-height:${height}%" title="${safe(item.day)} · ${safe(fmtNum(count))} transactions"><span>${safe(fmtNum(count))}</span></i>`;
    }).join('')}</div><div class="wallet-timeline-meta"><span>${safe(buckets[0]?.day || '')}</span><span>${safe(buckets.at(-1)?.day || '')}</span></div>`;
  }
  ...
  function clearResults(options = {}) {
    ...
    const priorAddress = state.address || String(profile?.publicWallet || '').trim();
    state.overview = null;
    state.activity = null;
    state.address = null;
  ...
  async function analyzeWallet(input, options = {}) {
    ...
    const requestedRange = RANGE_LABELS[options.range] ? options.range : state.range;
    ...
    const previousAddress = String(state.address || profile?.publicWallet || '').trim();
    if (isValidSolanaAddress(previousAddress) && previousAddress !== address) clearResults();
    global.BBRSJourney?.unlock(address);

    if (options.preferCache === true && restoreCachedWallet(address)) {
      setWalletLoading(false);
      if (options.navigate !== false) openIntelligenceResults();
      global.dispatchEvent?.(new CustomEvent('bbrs:wallet-analysis-complete', { detail: { address, range: state.range, cached: true, partial: false } }));
      return true;
    }
    ...
    await Promise.all([overviewTask, activityTask]);
    ...
    setText('trackPhase', errors.length ? 'Partial data' : '');
    global.toast?.(errors.length ? 'Partial wallet data loaded' : 'Wallet analysis updated');
    if (options.navigate !== false) openIntelligenceResults();
    global.dispatchEvent?.(new CustomEvent('bbrs:wallet-analysis-complete', { detail: { address, range: state.range, cached: false, partial: errors.length > 0 } }));
    return true;
  }

  function restoreSavedWalletOnce() {
    const address = String(profile?.publicWallet || '').trim();
    ...
    if (!restored) analyzeWallet(address, { silent: true, navigate: false, preferCache: true }).catch(() => {});
  }

  function initTrackData() {
    el('trackAnalyze')?.addEventListener('click', () => analyzeWallet(undefined, { navigate: true }));
    el('walletInput')?.addEventListener('keydown', event => { if (event.key === 'Enter') analyzeWallet(undefined, { navigate: true }); });
    document.querySelectorAll('#trackRangeTabs .tab').forEach(tab => tab.addEventListener('click', () => {
      ...
      if (state.address && !restoreCachedWallet(state.address)) analyzeWallet(state.address, { navigate: false });
    }));
    clearResults();
  ...
  global.BBRWalletAnalytics = { isValidSolanaAddress, restoreCachedWallet, analyze: analyzeWallet, getState: () => ({ ...state }) };
})(window);
