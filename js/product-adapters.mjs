...
function adapter(id, definition = {}) {
  ...
  return Object.freeze({
    ...
    status: typeof definition.status === 'function'
      ? definition.status
...
export class ProductAdapterRegistry {
  ...
  status() {
    return Object.freeze([...this.#adapters.values()].map((item) => Object.freeze({
      id: item.id,
      ...item.status()
    })));
  }
...
export function legacyViewAdapter({
  viewId,
  activateView,
  beforeActivate,
  afterActivate
  ...
  return Object.freeze({
    activate(context) {
      ...
      const result = activateView(viewId, context);
