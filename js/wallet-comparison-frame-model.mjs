...
function through(events=[],chainTime){if(chainTime==null)return Object.freeze([]);return Object.freeze(events.filter(event=>finite(event?.timestamp)!=null&&finite(event.timestamp)<=chainTime));}
...
export function buildWalletComparisonFrameModel({bundle={},manifest={},renderPlan=[]}={},frame=0){
  ...
  const events=replayEvents(bundle),walletA=state.walletA||bundle?.comparison?.walletA?.wallet||'',walletB=state.walletB||bundle?.comparison?.walletB?.wallet||'';
