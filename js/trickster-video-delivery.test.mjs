...
test('describes a real encoded video as save-ready',()=>{
  const state=describeVideoDelivery({videoReady:true,blob:{size:1},extension:'mp4',width:1080,height:1920,fps:30});
  assert.equal(state.mode,'ready');assert.equal(state.extension,'mp4');assert.match(state.summary,/1080×1920/);
});
