...
import { buildMaze, parseMaze, CELL } from './maze.js';
...
/** Fast mobile street: boxes + collision proxies, no kit/interiors/dressing. */
function buildLiteStreet(A) {
  const box = A.cache('lite-box', () => new THREE.BoxGeometry(1, 1, 1));
  A.addBox('asphalt', box, 0, -0.12, -8, 0, 14, 0.24, 110);
  A.box('concrete', 0, -0.12, -8, 14, 0.24, 110);
  A.addBox('sand', box, 0, -0.18, -8, 0, 80, 0.12, 120);
  A.box('dirt', 0, -0.2, -8, 80, 0.1, 120);

  A.addBox('concrete', box, -8.2, 0.1, -8, 0, 2.6, 0.2, 110);
  A.addBox('concrete', box, 8.2, 0.1, -8, 0, 2.6, 0.2, 110);

  const blocks = [
    { x: -14, z: 24, w: 10, d: 14, h: 7.5, key: 'plaster_cream' },
    { x: -14.5, z: 6, w: 11, d: 12, h: 8.5, key: 'plaster_sand' },
    { x: -14, z: -12, w: 10, d: 14, h: 7, key: 'plaster_cream' },
    { x: -13.5, z: -30, w: 11, d: 12, h: 9, key: 'plaster_blue' },
    { x: 14, z: 22, w: 10, d: 14, h: 8, key: 'plaster_sand' },
    { x: 14.5, z: 4, w: 11, d: 12, h: 7.2, key: 'plaster_cream' },
    { x: 14, z: -14, w: 10, d: 14, h: 8.8, key: 'plaster_pink' },
    { x: 13.8, z: -32, w: 11, d: 12, h: 7.6, key: 'plaster_sand' },
  ];
  for (const b of blocks) {
    A.addBox(b.key, box, b.x, b.h * 0.5, b.z, 0, b.w, b.h, b.d);
    A.box('concrete', b.x, b.h * 0.5, b.z, b.w, b.h, b.d);
  }

  A.addBox('concrete', box, -5.2, 4.2, -48, 0, 4.2, 8.4, 2.4);
  A.addBox('concrete', box, 5.2, 4.2, -48, 0, 4.2, 8.4, 2.4);
  A.addBox('concrete', box, 0, 8.2, -48, 0, 14.8, 2.2, 2.4);
  A.box('concrete', -5.2, 4.2, -48, 4.2, 8.4, 2.4);
  A.box('concrete', 5.2, 4.2, -48, 4.2, 8.4, 2.4);
  A.box('concrete', 0, 8.2, -48, 14.8, 2.2, 2.4);

  A.addBox('brick', box, -28, 3, -8, 0, 1.2, 6, 110);
  A.addBox('brick', box, 28, 3, -8, 0, 1.2, 6, 110);
  A.box('concrete', -28, 3, -8, 1.2, 6, 110);
  A.box('concrete', 28, 3, -8, 1.2, 6, 110);
}
...
/** Spawn points filled from the maze after build. */
const SPAWNS = [];
...
export class WorldSystem {
  ...
  async init(ctx) {
    ...
    ctx.onStatus?.('Raising compound maze');
    const maze = buildMaze(A, rng);
    this.maze = maze;
    this.buildings = [];
    ...
    const start = maze.start ?? { x: 0, z: 20, yaw: Math.PI };
    const boss = maze.boss ?? { x: 0, z: -20, yaw: 0 };
    this.spawnPoints = [
      { position: A.toWorld(start.x, 0.08, start.z - 1.4), yaw: start.yaw + LEVEL_YAW, tag: 'start' },
      { position: A.toWorld(boss.x, 0.05, boss.z), yaw: boss.yaw + LEVEL_YAW, tag: 'extract' },
    ];
    this.extractPoint = A.toWorld(boss.x, 0.2, boss.z);
    this.enemySpots = (maze.enemySpots ?? []).map((p) => ({
      position: A.toWorld(p.x, 0.05, p.z),
      yaw: 0,
      tag: p.ch === 'x' ? 'deadend' : 'patrol',
    }));
    this.bounds = new THREE.Box3(
      new THREE.Vector3(-maze.size.width * 0.5 - 8, -2, -maze.size.depth * 0.5 - 8),
      new THREE.Vector3(maze.size.width * 0.5 + 8, 18, maze.size.depth * 0.5 + 8)
  ...
  _addLights(A) {
    ...
    for (const p of A.lampAnchors) {
      const l = new THREE.PointLight(0xffb765, 7, 22, 2);
  ...
  update(dt, ctx) {
    ...
    if (Math.abs(mix - this._lampMix) > 0.01) {
      this._lampMix = mix;
      for (let i = 0; i < this.lamps.length; i++) this.lamps[i].intensity = 7 + 12 * mix;
  ...
  /** True where a character can stand outdoors (corridor, arena). */
  isOpen(x, z, margin = 0.4) {
    const maze = this.maze;
    if (!maze?.open) return true;
    const p = this.worldToLevel(x, 0, z, this._v);
    const half = CELL * 0.5 - margin;
    for (const c of maze.open) {
      if (Math.abs(c.x - p.x) <= half && Math.abs(c.z - p.z) <= half) return true;
    }
    return false;
  }
