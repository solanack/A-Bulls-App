...
/**
 * Pick unique patrol anchors with a protected player-start radius, then use
 * farthest-point sampling so the opening garrison occupies the whole maze.
 */
export function selectSpreadSpawns(spots, start, count, minStartDistance = 20) {
  const all = (spots ?? [])
    .filter(entry => entry?.position)
    .map(entry => ({ s: entry, d: start ? entry.position.distanceTo(start) : minStartDistance + 1 }));
  let remaining = all.filter(entry => entry.d >= minStartDistance);
  if (remaining.length < Math.min(count, 8)) {
    remaining = all.filter(entry => entry.d >= Math.max(12, minStartDistance * 0.72));
  }
  const selected = [];
  while (remaining.length && selected.length < count) {
    ...
    for (let i = 0; i < remaining.length; i++) {
      ...
      const separation = selected.length
        ? Math.min(...selected.map(other => entry.s.position.distanceTo(other.s.position)))
        : entry.d;
      const score = separation + entry.d * 0.18;
      if (score > bestScore) {
        bestScore = score;
        bestIndex = i;
      }
...
export class AiSystem {
  ...
  async init(ctx) {
    ...
    this._bootNav(ctx);
    // compileAsync can hang forever on software/WebGL in a preview iframe.
    // Skip shader pre-warm on mobile/lite; first combat frame compiles instead.
    if (!ctx.config.mobile) {
      await Promise.race([
        this.prewarmMaterials(),
  ...
  /**
   * Garrison the maze: 20 hostiles along corridors and a commander at the gate.
   */
  populate(opts = {}) {
    const world = this.ctx.peek('world');
    if (!this.grid) return 0;

    const spots = world?.enemySpots?.length
      ? world.enemySpots
      : (world?.spawnPoints ?? []).slice(1);
    if (!spots.length) return 0;

    const variants = ['vanguard', 'irregular', 'breacher'];
    const drops = ['ammo', 'health', 'weapon', 'ammo', 'ammo'];
    const want = opts.count ?? 20;
    const start = world?.spawn?.(0)?.position;
    const ranked = selectSpreadSpawns(spots, start, want, opts.minSpawnDistance ?? 20);
    ...
    for (let i = 0; i < want; i++) {
      ...
      const anchor = entry.s;
      const jitterA = this.rng.range(0, Math.PI * 2);
      const jitterR = this.rng.range(0.2, 1.1);
      ...
      const patrol = [p.clone()];
      const other = ranked[(i + Math.max(3, Math.floor(ranked.length / 3))) % ranked.length]?.s?.position;
      if (other) patrol.push(other.clone());
      const a = this.spawn(variants[i % variants.length], p, this.rng.signed() * Math.PI, {
        patrol,
        dropKind: drops[i % drops.length],
        name: `HOSTILE ${i + 1}`,
      });
      squad.add(a);
      made++;
    ...
    const extract = world?.extractPoint ?? world?.spawn?.(1)?.position;
    if (extract) {
      const bp = extract.clone();
      const ci = this.grid.nearest(bp.x, bp.z, bp.y, 8, 1.6);
      if (ci >= 0) {
        bp.set(
          this.grid.worldX(ci % this.grid.nx),
          this.grid.floor[ci],
        ...
        bp.y = this.groundAt(bp.x, bp.z, bp.y + 6);
      }
      const boss = this.spawn('breacher', bp, 0, {
        boss: true,
        scale: 1.42,
        health: 900,
        weaponDamage: 36,
    ...
    const nearest = ranked.reduce((distance, entry) => Math.min(distance, entry.d), Infinity);
    console.info(`[ai] garrison: ${made} (20 hostiles + commander), nearest start ${nearest.toFixed(1)}m`);
    return made;
  }
