...
import { AimAssist } from './aim-assist.js';
...
export class PlayerSystem {
  ...
  async init(ctx) {
    ...
    on('player:turn-180', () => this.turnAround());
    ...
    this._installControlsProbe();
  }

  _installControlsProbe() {
    const self = this;
    const wrap = (a) => Math.atan2(Math.sin(a), Math.cos(a));
    globalThis.__controlsTest = {
      getYaw: () => self.movement.yaw,
      getPitch: () => self.movement.pitch,
      getSpeed: () => self.movement.horizontalSpeed,
      getMove: () => ({ x: self.movement.cmd.moveX, y: self.movement.cmd.moveY }),
      setKeys(codes) {
        const input = self.ctx.input;
        input.down.clear();
        for (const c of codes) input.down.add(c);
      },
      setMove(x, y) {
        self.ctx.input.setTouchMove(x, y);
      },
      setSteer(v) {
        self.ctx.input.addTouchLook(-v * 24, 0);
      },
      wrap,
    };
  }
  ...
  _consumeLook(dt) {
    ...
    if (stick.lookX || stick.lookY) {
      ...
      const turnSensitivity = Number.isFinite(cfg.turnSensitivity)
        ? clamp(cfg.turnSensitivity, 0.35, 2.25)
        : 1;
      dYaw -= stick.lookX * rate * dt * turnSensitivity;
    ...
    dYaw += this.aimAssist?.update(dt, m, Math.abs(input.look.x) + Math.abs(input.look.y)) ?? 0;
  ...
  update(dt, ctx) {
    ...
    if (this.controlEnabled) {
      this.rig.applyTo(ctx.camera);
      ctx.input.markTouchLookApplied?.(performance.now());
    }
  ...
  /** Dedicated mobile quick-turn: rotate the body and camera exactly 180°. */
  turnAround() {
    if (!this.controlEnabled || this.health?.dead || !this.movement) return false;
    const m = this.movement;
    m.yaw = Math.atan2(Math.sin(m.yaw + Math.PI), Math.cos(m.yaw + Math.PI));
    m.yawRate = 0;
    this._prevYaw = m.yaw;
    return true;
  }
