...
export class WeaponSystem {
  ...
  get weaponIds() {
    return [...this.owned].filter((id) => this.states.has(id));
  }
  ...
  addReserve(amount, id = this.activeId) {
    const s = this.states.get(id) ?? this.state;
  ...
  grantWeapon(id) {
    ...
    if (s) s.reserve = Math.max(s.reserve, Math.floor(s.def.reserve * 0.35));
    if (fresh) this.setWeapon(id);
    return true;
  }
  ...
  getHudState() {
    ...
    h.lethalCount = this.lethalCount;
    return h;
  }

  setLethalCount(count) {
    this.lethalCount = Math.max(0, Number(count) | 0);
  }
