...
export async function persistUniverseObservations(env = {}, inputs = []) {
  ...
  for (const input of inputs.slice(0, 1000)) {
    ...
    `).bind(item.eventId,item.entityKind,item.entityId,item.category,item.observedAt,item.slot,
      item.commitment,item.magnitudeBand,item.source,JSON.stringify(item.evidence)).run();
    written += 1;
  }
  return written;
}

export async function universeSnapshot(env = {}, { windowSeconds = 60, limit = 2500, now = Math.floor(Date.now()/1000) } = {}) {
  const db = intelligenceDb(env);
  const window = Math.max(10, Math.min(120, Math.trunc(n(windowSeconds) || 60)));
  const cap = Math.max(1, Math.min(5000, Math.trunc(n(limit) || 2500)));
  const from = now - window;
  if (!db) return { windowStart:from,windowEnd:now,observedEventCount:0,particles:[],sources:[],samplingPolicy:'no database binding',coverageStatement:'No live observations available.' };
  ...
  return {
    ...
    samplingPolicy:rows.length>cap?'35% magnitude-priority plus category-balanced sample':'all bounded observations',
