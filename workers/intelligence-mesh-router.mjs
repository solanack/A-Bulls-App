...
export async function handleIntelligenceMeshRequest(request, env = {}) {
  ...
  if (url.pathname === '/api/intelligence/chain-radar' && request.method === 'GET') {
    return json({ ok:true, state:'descriptive-anomaly-feed', anomalies:await chainRadar(env), disclaimer:'Observed anomalies are descriptive and are not price predictions or recommendations.' });
  }
  if (url.pathname === '/api/intelligence/chain-weather' && request.method === 'GET') {
    return json({ ok:true, weather:await chainWeather(env), disclaimer:'Visualization of observed aggregate chain activity; not a market forecast.' });
  }
  ...
  if (url.pathname === '/api/intelligence/index-coverage' && request.method === 'POST') {
    ...
    const started = Date.now(); const coverage = await coverageForWallet(env, wallet);
  ...
  if (url.pathname === '/api/intelligence/wallet-dna' && request.method === 'POST') {
    const payload = await body(request); const wallet = s(payload.wallet || payload.address);
    if (!validWallet(wallet)) return json({ ok:false, error:'invalid_public_wallet' },400);
    const started=Date.now(); const dna=await walletDna(env,wallet); await recordDemand(env,'wallet-dna','wallet',wallet,Date.now()-started);
    return json({ ok:true, wallet, dna });
  }
  if (url.pathname === '/api/intelligence/time-machine' && request.method === 'POST') {
    const payload=await body(request); const wallet=s(payload.wallet||payload.address);
    if(!validWallet(wallet)) return json({ok:false,error:'invalid_public_wallet'},400);
    const started=Date.now(); const result=await timeMachine(env,wallet,payload.limit); await recordDemand(env,'time-machine','wallet',wallet,Date.now()-started);
    return json({ok:true,...result});
  }
  if (url.pathname === '/api/intelligence/constellation' && request.method === 'POST') {
    const payload=await body(request); const wallet=s(payload.wallet||payload.address);
    if(!validWallet(wallet)) return json({ok:false,error:'invalid_public_wallet'},400);
    const edges=await constellation(env,wallet,payload.limit); return json({ok:true,wallet,edges,disclaimer:'Observed public transaction relationships do not prove common ownership or identity.'});
  }
  if (url.pathname === '/api/intelligence/museum' && request.method === 'POST') {
    const payload=await body(request); const wallet=s(payload.wallet||payload.address);
    if(!validWallet(wallet)) return json({ok:false,error:'invalid_public_wallet'},400);
    return json({ok:true,museum:await museum(env,wallet)});
  }
  if (url.pathname === '/api/intelligence/chain-lens' && request.method === 'POST') {
    const payload=await body(request); const wallet=s(payload.wallet||payload.address); const signature=s(payload.signature);
    if(!validWallet(wallet)||!signature) return json({ok:false,error:'wallet_and_signature_required'},400);
  ...
  if (url.pathname === '/api/intelligence/candles' && request.method === 'POST') {
    const payload=await body(request); const mint=s(payload.mint); const quoteMint=s(payload.quoteMint||payload.quote_mint);
    if(!mint||!quoteMint) return json({ok:false,error:'mint_and_quote_required'},400);
    const rows=await candles(env,mint,quoteMint,payload.bucketSeconds||payload.bucket_seconds||60,payload.limit||300);
    return json({ok:true,mint,quoteMint,candles:rows.reverse(),source:'observed-on-chain-swaps'});
  }
  if (url.pathname === '/api/intelligence/verification' && request.method === 'POST') {
    const payload = await body(request); const wallet = s(payload.wallet || payload.address); const signature = s(payload.signature);
    if (!validWallet(wallet) || !signature) return json({ ok: false, error: 'wallet_and_signature_required' }, 400);
    const started = Date.now(); const verification = await verificationForSignature(env, wallet, signature);
  ...
  if (url.pathname === '/api/intelligence/market-sequence' && request.method === 'POST') {
    const payload = await body(request); const scopeType = s(payload.scopeType || payload.scope_type); const scopeValue = s(payload.scopeValue || payload.scope_value);
    if (!scopeType || !scopeValue) return json({ ok: false, error: 'scope_required' }, 400);
    const started = Date.now(); const events = await marketSequence(env, scopeType, scopeValue, payload.limit);
