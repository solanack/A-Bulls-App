...
function fakeDb(rowsByWallet){
  return {prepare(){return{bind(walletArg){return{all:async()=>({results:rowsByWallet[walletArg]||[]})}}}}};
}
...
test('finds tokens observed in both wallets without implying coordination',async()=>{
  const db=fakeDb({
    [wallet]:[
      {mint,event_count:9,trade_count:4,first_event:100,last_event:500,observed_token_flow:42,max_confidence:.9},
      {mint:mintOnlyA,event_count:2,trade_count:1,first_event:200,last_event:300,observed_token_flow:4,max_confidence:.7}
    ],
    [walletB]:[{mint,event_count:7,trade_count:3,first_event:150,last_event:600,observed_token_flow:31,max_confidence:.85}]
  });
  const result=await buildCommonTokenIndex({},wallet,walletB,{db});
  assert.equal(result.commonTokenCount,1);assert.equal(result.commonTokens[0].mint,mint);assert.equal(result.commonTokens[0].combinedTradeCount,7);assert.equal(result.commonTokens[0].overlapFrom,150);assert.equal(result.commonTokens[0].overlapTo,500);assert.match(result.disclosure,/does not prove shared ownership, coordination/);
});
