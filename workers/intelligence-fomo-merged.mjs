...
async function auditPayload(env){
  ...
  const [summary,positionHandles,tradeHandles,observedHandles]=await Promise.all([
    first(`SELECT COUNT(*) traders,SUM(CASE WHEN reported_pnl_usd IS NOT NULL THEN 1 ELSE 0 END) with_pnl,SUM(CASE WHEN solana_wallet IS NOT NULL AND TRIM(solana_wallet)<>'' THEN 1 ELSE 0 END) with_solana_wallet,SUM(CASE WHEN top_tokens_json IS NOT NULL AND top_tokens_json<>'' AND top_tokens_json<>'[]' THEN 1 ELSE 0 END) with_mapped_tokens FROM fomo_traders WHERE current_rank BETWEEN 1 AND 50`),
    first(`SELECT COUNT(DISTINCT handle) count FROM fomo_trader_positions`),
    first(`SELECT COUNT(DISTINCT handle) count FROM fomo_trader_trades`),
    first(`SELECT COUNT(DISTINCT t.handle) count FROM fomo_traders t JOIN bull_wallet_events e ON e.wallet=t.solana_wallet WHERE t.current_rank BETWEEN 1 AND 50`),
  ]);
