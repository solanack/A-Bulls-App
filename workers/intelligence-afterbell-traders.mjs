...
function eventKey(event){return s(event.txId)||[s(event.wallet),n(event.blockTime),s(event.side),n(event.amount),s(event.source)].join(':');}
export function normalizeAfterbellEvents(rows=[]){
  const byKey=new Map();
  for(const raw of Array.isArray(rows)?rows:[]){
    const wallet=canonicalChainAddress('solana',s(raw.wallet)),side=s(raw.side).toLowerCase(),amount=Math.abs(n(raw.amount)),blockTime=Math.trunc(n(raw.blockTime)),txId=s(raw.txId)||null,priceUsd=finite(raw.priceUsd),priceSol=finite(raw.priceSol),sourceKind=s(raw.sourceKind)||'provider-reported';
    if(!wallet||!['buy','sell'].includes(side)||!amount||!blockTime)continue;
    const event=Object.freeze({wallet,side,amount,blockTime,txId,priceUsd:priceUsd!=null&&priceUsd>0?Math.abs(priceUsd):null,priceSol:priceSol!=null&&priceSol>0?Math.abs(priceSol):null,source:s(raw.source)||'indexed-d1',sourceKind});
...
export function rankAfterbellTraders(events=[],{from,to,limit=AFTERBELL_MAX_TRADERS}={}){
  ...
  for(const event of normalized){if(event.blockTime<start||event.blockTime>end)continue;const row=byWallet.get(event.wallet)||{wallet:event.wallet,txIds:new Set(),eventCount:0,buyCount:0,sellCount:0,lastObservedAt:0,sources:new Set(),sourceKinds:new Set()};row.txIds.add(event.txId||eventKey(event));row.eventCount++;if(event.side==='buy')row.buyCount++;else row.sellCount++;row.lastObservedAt=Math.max(row.lastObservedAt,event.blockTime);row.sources.add(event.source);row.sourceKinds.add(event.sourceKind);byWallet.set(event.wallet,row);}
  const result=[];
  for(const row of byWallet.values()){const walletEvents=normalized.filter(event=>event.wallet===row.wallet&&event.blockTime<=end),pnlUsd=realizedForUnit(walletEvents,start,end,'usd'),pnlSol=realizedForUnit(walletEvents,start,end,'sol');result.push(Object.freeze({wallet:row.wallet,transactionCount:row.txIds.size,eventCount:row.eventCount,buyCount:row.buyCount,sellCount:row.sellCount,realizedPnlUsd:pnlUsd,realizedPnlSol:pnlSol,lastObservedAt:row.lastObservedAt*1000,sourceKind:row.sourceKinds.has('observed-fact')?'observed':'provider-reported',sources:Object.freeze([...row.sources])}));}
  result.sort((a,b)=>b.transactionCount-a.transactionCount||(Math.abs(b.realizedPnlUsd??b.realizedPnlSol??-Infinity)-Math.abs(a.realizedPnlUsd??a.realizedPnlSol??-Infinity))||b.lastObservedAt-a.lastObservedAt||a.wallet.localeCompare(b.wallet));
...
async function readRows(db,mint,from,to){
  ...
  const chain=await all(db.prepare(`SELECT wallet_address wallet,tx_id txId,LOWER(side) side,ABS(COALESCE(amount,0)) amount,price_usd priceUsd,NULL priceSol,block_time blockTime,source,source_kind sourceKind FROM intelligence_chain_events_v2 WHERE chain_key='solana' AND asset_address=? AND block_time BETWEEN ? AND ? AND wallet_address IS NOT NULL AND LOWER(side) IN ('buy','sell') ORDER BY block_time ASC LIMIT 10000`).bind(mint,lookback,to));rows.push(...chain);
  const native=await all(db.prepare(`SELECT wallet,signature txId,CASE WHEN token_delta>0 THEN 'buy' ELSE 'sell' END side,ABS(token_delta) amount,NULL priceUsd,CASE WHEN ABS(token_delta)>0 AND ABS(sol_delta)>0 THEN ABS(sol_delta/token_delta) ELSE NULL END priceSol,block_time blockTime,source,'observed-fact' sourceKind FROM bull_wallet_events WHERE mint=? AND block_time BETWEEN ? AND ? AND wallet IS NOT NULL AND token_delta<>0 ORDER BY block_time ASC LIMIT 10000`).bind(mint,lookback,to));rows.push(...native);return rows;
}

export async function readAfterbellTraders(env={},mintInput='',options={}){
  const db=intelligenceDb(env),mint=canonicalChainAddress('solana',s(mintInput));if(!mint)return Object.freeze({ok:false,coverage:'empty',error:'invalid_xstock_mint',mint:s(mintInput),items:[],disclosure:'A valid Solana xStock mint is required. No trader list was invented.'});if(!db)return Object.freeze({ok:false,coverage:'degraded',error:'database_unavailable',mint,items:[],disclosure:'The Intelligence D1 binding is unavailable. No trader list or PnL was invented.'});
  const window=options.from&&options.to?Object.freeze({from:Math.trunc(n(options.from)),to:Math.trunc(n(options.to)),scheduledEnd:Math.trunc(n(options.to)),live:false,timezone:ZONE,label:'CUSTOM AFTERBELL WINDOW',calendarCoverage:'caller supplied'}):afterbellWindow(options.nowMs),rows=await readRows(db,mint,window.from,window.to),items=rankAfterbellTraders(rows,{from:window.from,to:window.to,limit:options.limit});
  return Object.freeze({ok:true,coverage:items.length?'fresh':'empty',mint,window,items,method:'afterbell-unique-transactions+bounded-fifo-realized-pnl-v1',disclosure:items.length?'Rank is unique retained transactions during the Afterbell window. Realized PnL is shown only when retained FIFO acquisition basis and sale price are complete in the available evidence; otherwise it stays unavailable. No identity, skill, ownership, or recommendation claim is made.':'No retained xStock wallet trades are indexed in this Afterbell window. Empty coverage stays empty; no trader or PnL was invented.'});
}

export async function handleAfterbellTradersRequest(request,env={}){
  const url=new URL(request.url);if(url.pathname!==AFTERBELL_TRADERS_PATH)return null;if(request.method!=='GET')return json({ok:false,error:'method_not_allowed'},405,'no-store');const mint=s(url.searchParams.get('mint')),limit=Math.max(1,Math.min(AFTERBELL_MAX_TRADERS,Math.trunc(n(url.searchParams.get('limit'))||AFTERBELL_MAX_TRADERS))),from=finite(url.searchParams.get('from')),to=finite(url.searchParams.get('to')),body=await readAfterbellTraders(env,mint,{limit,from,to}),status=body.error==='invalid_xstock_mint'?400:body.error==='database_unavailable'?503:200;return json(body,status,status===200?'public, max-age=15, stale-while-revalidate=30':'no-store');
}
