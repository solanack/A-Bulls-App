...
async function earliestReplayTime(db,chain,wallets,mint){
  let earliest=null;
...
async function resolveWindow(db,chain,wallets,mint,input){
  const now=Math.floor(Date.now()/1000),rawTo=input.to??input.endTime,rawFrom=input.from??input.startTime,to=Math.max(0,Math.trunc(n(rawTo??now)));
  let from;if(rawFrom!=null&&rawFrom!=='')from=Math.max(0,Math.trunc(n(rawFrom)));else from=(await earliestReplayTime(db,chain,wallets,mint))??Math.max(0,to-30*86400);
  const low=Math.min(from,to),high=Math.max(from,to);if(high-low>MAX_WINDOW_SECONDS)throw new RangeError('replay_window_too_large');
  return Object.freeze({from:low,to:high,bucketSeconds:adaptiveReplayBucketSeconds(low,high,input.bucketSeconds)});
}
...
async function buildSolanaReplayBundle(env,db,subject,window,input){
  ...
  const verificationCounts={observed:0,confirmed:0,finalized:0,verified:0,unknown:0};for(const event of events)verificationCounts[event.verification]=(verificationCounts[event.verification]||0)+1;const complete=coverages.length>0&&coverages.every(item=>n(item?.complete_to_genesis)>0),coverageStatement=complete?'Indexed wallet histories report complete-to-genesis coverage; this Replay is bounded to the selected trade window and token.':'Replay uses currently indexed Solana evidence while the Intelligence Mesh continues bounded history hydration.';
  return Object.freeze({schemaVersion:'replay-bundle-v2',generatedAt:Date.now(),subject:Object.freeze({kind:wallets.length===2?'wallet-comparison':'wallet-token',chain:'solana',assetId:chainQualifiedId('solana',mint),wallets:Object.freeze(wallets),mint,quoteMint:quoteMint||null}),window:Object.freeze({from,to,startTime:from*1000,endTime:to*1000,bucketSeconds}),eventCount:events.length,events:Object.freeze(events),candles:Object.freeze(candles),coverage:Object.freeze({wallets:Object.freeze(coverages),complete,statement:coverageStatement}),verification:Object.freeze(verificationCounts),sources:Object.freeze([...sourceSet].sort()),caveats:Object.freeze(['Public-chain observations only; wallet relationships do not prove identity or common ownership.',quoteMint?'Execution price is shown only when indexed route evidence directly supports the selected base/quote pair.':'No quote mint was supplied, so execution prices are not inferred.',candles.length?'Candles are indexed market observations with their own confidence/source metadata.':'No indexed OHLC series is available for this selection yet; no price series was invented.','What-if overlays are historical counterfactuals, not predictions or claims of achievable execution.'])});
}
...
export async function handleReplayBundleRequest(request,env={}){
  ...
  try{
    ...
    if(chain==='solana'&&!bundle.coverage.complete){for(const wallet of bundle.subject.wallets){const job=await queueHistoryJob(env,wallet,{pageSize:25,requestedFrom:bundle.window.from,requestedTo:bundle.window.to});indexingJobs.push(Object.freeze({wallet,...job}));}}
    ...
    const indexing=chain==='solana'?Object.freeze({requested:pendingJobs.length>0,state:bundle.coverage.complete?'not-required':pendingJobs.length?'queued-or-running':windowComplete?'window-ready':'not-required',windowComplete,jobs:Object.freeze(indexingJobs),disclosure:pendingJobs.length?'Missing or partial Solana wallet coverage was queued automatically for this exact Replay window.':windowComplete&&!bundle.coverage.complete?'The selected Replay window has completed bounded Solana history hydration.':'No additional Solana wallet-history job was required for this request.'}):Object.freeze({requested:false,state:'chain-qualified',windowComplete:false,jobs:Object.freeze([]),disclosure:`${chain} Replay reads only the ${chain} evidence spine and market cache. It is not routed through Solana RPC or the Solana history scheduler.`});
    const marketHydration=await replayMarketHydrationState(env,{chain,mint:bundle.subject.mint,quoteMint:bundle.subject.quoteMint,from:bundle.window.from,to:bundle.window.to,bucketSeconds:bundle.window.bucketSeconds,candleCount:bundle.candles.length}),responseBundle=Object.freeze({...bundle,indexing,marketHydration}),requestedJobIds=pendingJobs.map(job=>Math.trunc(n(job.jobId))).filter(id=>id>0);
    ...
    if(marketHydration.requested&&env.__EXECUTION_CTX?.waitUntil)env.__EXECUTION_CTX.waitUntil(hydrateReplayMarketCandles(env,{chain,mint:bundle.subject.mint,quoteMint:bundle.subject.quoteMint,from:bundle.window.from,to:bundle.window.to,bucketSeconds:bundle.window.bucketSeconds}).catch(()=>null));
...
export const __replayBundleContract=Object.freeze({chainQualified:true,solanaHistorySchedulerOnlyForSolana:true,adaptiveHistoricalWindow:true,maxWindowYears:15,noSyntheticData:true});
