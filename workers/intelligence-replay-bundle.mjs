...
export function providerReportedFomoTradeEvents(row={},chainOverride='',walletOverride='',mintOverride=''){
  ...
  const common={signature:null,wallet,token:mint,slot:null,kind:'trade',tokenDelta:nullable(row.amount),solDelta:null,feeLamports:null,counterparty:null,programId:null,confidence:.65,verification:'provider-reported',sourceKind:'provider-reported',sources:Object.freeze([s(row.source)||'fomoapi.io/trades'])},evidenceBase={handle:s(row.handle),tradeId,status:s(row.status)||null,reportedRealizedPnlUsd:nullable(row.realized_pnl_usd),reportedUnrealizedPnlUsd:nullable(row.unrealized_pnl_usd)};
  const events=[Object.freeze({...common,id:`fomo-provider:${chain}:${handle}:${tradeId}:entry`,timestamp:createdAt*1000,side:'buy',price:null,priceUsd:nullable(row.avg_entry_price),evidence:Object.freeze({...evidenceBase,phase:'entry'})})];
  if(closedAt>=createdAt&&closedAt>0)events.push(Object.freeze({...common,id:`fomo-provider:${chain}:${handle}:${tradeId}:exit`,timestamp:closedAt*1000,side:'sell',price:null,priceUsd:nullable(row.avg_exit_price),evidence:Object.freeze({...evidenceBase,phase:'exit'})}));
...
async function latestFomoTradeAnchor(db,chain,wallet,mint,to){
  const rows=chain==='solana'
    ?await all(db.prepare(`SELECT x.trade_id,x.chain,x.status,x.created_at,x.closed_at,x.source FROM fomo_trader_trades x JOIN fomo_traders t ON t.handle=x.handle WHERE t.solana_wallet=? AND x.token_address=? AND x.created_at IS NOT NULL AND x.created_at>0 AND x.created_at<=? ORDER BY x.created_at DESC LIMIT 50`).bind(wallet,mint,to))
