...
async function buildChainReplayBundle(db,subject,window,input){
  ...
  const marketContext=window.startResolved?await retainedExecutionContext(db,chain,wallets,mint,from,to):null;let effectiveQuote=quoteMint||marketContext?.quoteMint||null,candleRows=window.startResolved&&effectiveQuote?await loadChainReplayCandleRows(db,chain,mint,effectiveQuote,bucketSeconds,from,to):[];if(window.startResolved&&!candleRows.length){const retainedQuote=await retainedReplayQuote(db,chain,mint,from,to);if(retainedQuote&&retainedQuote!==effectiveQuote){effectiveQuote=retainedQuote;candleRows=await loadChainReplayCandleRows(db,chain,mint,effectiveQuote,bucketSeconds,from,to);}}
  const candles=candleRows.map(normalizeCandle);candles.flatMap(candle=>candle.sources||[]).forEach(source=>sourceSet.add(source));
