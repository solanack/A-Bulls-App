...
export function providerReportedFomoTradeEvents(row={},chainOverride='',walletOverride='',mintOverride=''){
  ...
  const common={signature:null,wallet,token:mint,slot:null,kind:'trade',tokenDelta:nullable(row.amount),solDelta:null,feeLamports:null,counterparty:null,programId:null,confidence:.65,verification:'provider-reported',sourceKind:'provider-reported',sources:Object.freeze(['fomoapi.io/trades'])},evidenceBase={handle:s(row.handle),tradeId,status:s(row.status)||null,reportedRealizedPnlUsd:nullable(row.realized_pnl_usd),reportedUnrealizedPnlUsd:nullable(row.unrealized_pnl_usd)};
