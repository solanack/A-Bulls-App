...
async function buildSolanaReplayBundle(env,db,subject,window,input){
  ...
  let effectiveQuote=quoteMint,candleRows=window.startResolved&&effectiveQuote?await loadSolanaReplayCandleRows(db,mint,effectiveQuote,bucketSeconds,from,to):[];if(window.startResolved&&!candleRows.length){const retainedQuote=await retainedReplayQuote(db,'solana',mint,from,to);if(retainedQuote&&retainedQuote!==effectiveQuote){effectiveQuote=retainedQuote;candleRows=await loadSolanaReplayCandleRows(db,mint,effectiveQuote,bucketSeconds,from,to);}}
...
async function buildChainReplayBundle(db,subject,window,input){
  ...
  let effectiveQuote=quoteMint,candleRows=window.startResolved&&effectiveQuote?await loadChainReplayCandleRows(db,chain,mint,effectiveQuote,bucketSeconds,from,to):[];if(window.startResolved&&!candleRows.length){const retainedQuote=await retainedReplayQuote(db,chain,mint,from,to);if(retainedQuote&&retainedQuote!==effectiveQuote){effectiveQuote=retainedQuote;candleRows=await loadChainReplayCandleRows(db,chain,mint,effectiveQuote,bucketSeconds,from,to);}}
  const candles=candleRows.map(normalizeCandle);candles.flatMap(candle=>candle.sources||[]).forEach(source=>sourceSet.add(source));
