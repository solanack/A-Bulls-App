...
async function resolveWindow(db,chain,wallets,mint,input){
  ...
  if(!anchor){return Object.freeze({from:requestedTo,to:requestedTo,bucketSeconds:60,startResolved:false,startSource:'unavailable',startSourceKind:'unavailable',startRef:null});}
  const resolvedTo=rawTo!=null&&rawTo!==''?requestedTo:Math.max(anchor.from,Math.trunc(n(anchor.to)||requestedTo)),low=Math.min(anchor.from,resolvedTo),high=Math.max(anchor.from,resolvedTo);if(high-low>MAX_WINDOW_SECONDS)throw new RangeError('replay_window_too_large');
