...
import { loadRobinhoodReferenceSeries } from './intelligence-robinhood-reference.mjs';
...
import { USD_QUOTE, hydrateReplayMarketCandles, replayMarketHydrationState } from './intelligence-replay-market-hydration.mjs';
...
function directExecution(routeRows=[],mint='',quoteMint=''){
  ...
  for(const row of routeRows){
    const inputMint=s(row.input_mint),outputMint=s(row.output_mint),input=Math.abs(n(row.input_amount)),output=Math.abs(n(row.output_amount));
    if(!(input>0&&output>0))continue;
    let candidate=null;
    // A missing requested quote (Afterbell) permits the *observed* counter-leg,
    // never an invented USD conversion. Explicit quotes retain exact-pair matching.
    if(outputMint===mint&&inputMint!==mint&&(!quoteMint||inputMint===quoteMint))candidate={side:'buy',price:input/output,baseAmount:output,quoteAmount:input,quoteMint:inputMint};
    else if(inputMint===mint&&outputMint!==mint&&(!quoteMint||outputMint===quoteMint))candidate={side:'sell',price:output/input,baseAmount:input,quoteAmount:output,quoteMint:outputMint};
    ...
    // When no quote was selected, prefer an actual direct USDC/USDT route for fill USD.
    const score=candidate.confidence+(!quoteMint&&USD_STABLE_MINTS.has(candidate.quoteMint)?1:0);
    if(!best||score>best.score)best={...candidate,score};
  }
  if(!best)return null;
  const {score,...execution}=best;
  return execution;
}
...
function normalizedChainReplayEvent(row){
  ...
  const blockTime=Math.max(0,Math.trunc(n(row.block_time))),sourceKind=s(row.source_kind)||'unknown',rawSide=s(row.side).toLowerCase(),side=rawSide==='entry'?'buy':rawSide==='exit'?'sell':rawSide||null,priceUsd=nullable(row.price_usd);
  return Object.freeze({id:s(row.event_id)||`${s(row.chain_key)}:${s(row.wallet_address)}:${blockTime}`,signature:s(row.tx_id)||null,wallet:s(row.wallet_address)||null,token:s(row.asset_address)||null,timestamp:blockTime*1000,slot:nullable(row.block_height),kind:s(row.event_class)||'event',side,price:null,priceUsd,tokenDelta:nullable(row.amount),solDelta:null,feeLamports:null,counterparty:s(evidence?.to)||null,programId:null,confidence:clamp(row.confidence,0,1),verification:sourceKind==='observed-fact'?'observed':'provider-reported',sourceKind,sources:Object.freeze(uniq([s(row.source)])),evidence:Object.freeze(evidence)});
}

function providerTransactionReference(chain,value){
  const raw=s(value);if(!raw)return null;return resolveChain(chain,{address:raw})?.kind==='svm'?(SOLANA_SIG_RE.test(raw)?raw:null):(EVM_TX_RE.test(raw)?raw.toLowerCase():null);
}

export function providerReportedFomoTradeEvents(row={},chainOverride='',walletOverride='',mintOverride=''){
  const chain=normalizeChainKey(chainOverride||row.chain||''),wallet=canonicalChainAddress(chain,walletOverride||row.wallet_address||''),mint=canonicalChainAddress(chain,mintOverride||row.token_address||''),createdAt=Math.max(0,Math.trunc(n(row.created_at))),closedAt=Math.max(0,Math.trunc(n(row.closed_at))),tradeId=s(row.trade_id),handle=s(row.handle).toLowerCase();
  if(!wallet||!mint||!tradeId||!createdAt)return Object.freeze([]);
  const closed=closedAt>=createdAt&&closedAt>0,entryRef=providerTransactionReference(chain,row.entry_tx_id??row.entryTxId??(!closed?row.tx_id:null)),exitRef=providerTransactionReference(chain,row.exit_tx_id??row.exitTxId??row.tx_id??(closed?tradeId:null)),common={wallet,token:mint,slot:null,kind:'trade',tokenDelta:nullable(row.amount),solDelta:null,feeLamports:null,counterparty:null,programId:null,confidence:.65,verification:'provider-reported',sourceKind:'provider-reported',sources:Object.freeze([s(row.source)||'fomoapi.io/trades'])},evidenceBase={handle:s(row.handle),tradeId,status:s(row.status)||null,reportedRealizedPnlUsd:nullable(row.realized_pnl_usd),reportedUnrealizedPnlUsd:nullable(row.unrealized_pnl_usd)};
  const events=[Object.freeze({...common,id:`fomo-provider:${chain}:${handle}:${tradeId}:entry`,signature:entryRef,timestamp:createdAt*1000,side:'buy',price:null,priceUsd:nullable(row.avg_entry_price),evidence:Object.freeze({...evidenceBase,phase:'entry',providerTransactionReference:entryRef})})];
  if(closed)events.push(Object.freeze({...common,id:`fomo-provider:${chain}:${handle}:${tradeId}:exit`,signature:exitRef,timestamp:closedAt*1000,side:'sell',price:null,priceUsd:nullable(row.avg_exit_price),evidence:Object.freeze({...evidenceBase,phase:'exit',providerTransactionReference:exitRef})}));
  return Object.freeze(events);
}

function appendUniqueReplayEvent(events,event){
  const duplicate=events.some(existing=>existing.id===event.id||(existing.sourceKind==='provider-reported'&&event.sourceKind==='provider-reported'&&existing.wallet===event.wallet&&existing.token===event.token&&existing.timestamp===event.timestamp&&existing.side===event.side));
...
async function retainedReplayQuote(db,chain,mint,from,to){
  ...
  const row=await first(db.prepare(`SELECT quote_asset_address quote,COUNT(*) count FROM intelligence_price_candles_v2 WHERE chain_key=? AND asset_address=? AND bucket_start BETWEEN ? AND ? GROUP BY quote_asset_address ORDER BY count DESC LIMIT 1`).bind(chain,mint,from,to));
  return canonicalChainAddress(chain,row?.quote)||null;
}
async function retainedExecutionContext(db,chain,wallets,mint,from,to){
  if(chain==='solana'){
    for(const wallet of wallets){const row=await first(db.prepare(`SELECT signature,wallet,venue,pool,input_mint,output_mint,input_amount,output_amount,block_time,confidence,source FROM intelligence_trade_routes WHERE wallet=? AND block_time BETWEEN ? AND ? AND (input_mint=? OR output_mint=?) ORDER BY CASE WHEN output_mint=? THEN 0 ELSE 1 END,confidence DESC,block_time ASC,hop_index ASC LIMIT 1`).bind(wallet,from,to,mint,mint,mint));if(row){const input=s(row.input_mint),output=s(row.output_mint),quote=input===mint?output:output===mint?input:null;if(quote)return Object.freeze({chain,wallet,txId:s(row.signature)||null,quoteMint:quote,poolAddress:s(row.pool)||null,dexId:s(row.venue)||null,blockTime:Math.trunc(n(row.block_time)),confidence:clamp(row.confidence,0,1),source:s(row.source)||'intelligence-trade-routes'});}}
    return null;
  }
  for(const wallet of wallets){const row=await first(db.prepare(`SELECT tx_id,wallet_address,quote_asset_address,pool_address,dex_id,block_time,confidence,source,side,evidence_json FROM intelligence_trade_execution_v2 WHERE chain_key=? AND wallet_address=? AND asset_address=? AND block_time BETWEEN ? AND ? ORDER BY CASE WHEN side='buy' THEN 0 ELSE 1 END,confidence DESC,block_time ASC LIMIT 1`).bind(chain,wallet,mint,from,to));if(row)return Object.freeze({chain,wallet,txId:s(row.tx_id)||null,quoteMint:canonicalChainAddress(chain,row.quote_asset_address)||null,poolAddress:s(row.pool_address)||null,dexId:s(row.dex_id)||null,blockTime:Math.trunc(n(row.block_time)),confidence:clamp(row.confidence,0,1),source:s(row.source)||'observed-trade-execution'});}
  return null;
}
...
async function latestFomoTradeAnchor(db,chain,wallet,mint,to){
  const rows=chain==='solana'
    ?await all(db.prepare(`SELECT x.trade_id,x.chain,x.status,x.created_at,x.closed_at,x.source,x.tx_id,x.entry_tx_id,x.exit_tx_id FROM fomo_trader_trades x JOIN fomo_traders t ON t.handle=x.handle WHERE t.solana_wallet=? AND x.token_address=? AND x.created_at IS NOT NULL AND x.created_at>0 AND x.created_at<=? ORDER BY x.created_at DESC LIMIT 50`).bind(wallet,mint,to))
...
async function buildSolanaReplayBundle(env,db,subject,window,input){
  ...
  for(const wallet of wallets){
    ...
    for(const route of routes){const key=s(route.signature);if(!bySignature.has(key))bySignature.set(key,[]);bySignature.get(key).push(route);}for(const row of rows){const execution=directExecution(bySignature.get(s(row.signature))||[],mint,quoteMint),event=normalizedReplayEvent(row,execution);event.sources.forEach(source=>sourceSet.add(source));appendUniqueReplayEvent(events,event);}for(const event of await loadFomoReplayEvents(db,'solana',wallet,mint,from,to)){event.sources.forEach(source=>sourceSet.add(source));appendUniqueReplayEvent(events,event);}coverages.push(await coverageForWallet(env,wallet));
  }
  events.sort((a,b)=>a.timestamp-b.timestamp||(a.slot||0)-(b.slot||0)||String(a.id||a.signature||'').localeCompare(String(b.id||b.signature||'')));
  const marketContext=window.startResolved?await retainedExecutionContext(db,'solana',wallets,mint,from,to):null;let effectiveQuote=quoteMint||marketContext?.quoteMint||null,candleRows=window.startResolved&&effectiveQuote?await loadSolanaReplayCandleRows(db,mint,effectiveQuote,bucketSeconds,from,to):[];if(window.startResolved&&!candleRows.length){const retainedQuote=await retainedReplayQuote(db,'solana',mint,from,to);if(retainedQuote&&retainedQuote!==effectiveQuote){effectiveQuote=retainedQuote;candleRows=await loadSolanaReplayCandleRows(db,mint,effectiveQuote,bucketSeconds,from,to);}}
  const candles=candleRows.map(normalizeCandle);candles.flatMap(candle=>candle.sources||[]).forEach(source=>sourceSet.add(source));
  ...
  return Object.freeze({schemaVersion:'replay-bundle-v2',generatedAt:Date.now(),subject:Object.freeze({kind:wallets.length===2?'wallet-comparison':'wallet-token',chain:'solana',assetId:chainQualifiedId('solana',mint),wallets:Object.freeze(wallets),mint,quoteMint:effectiveQuote||null}),marketContext:marketContext?Object.freeze(marketContext):null,window:Object.freeze({...window,startTime:from*1000,endTime:to*1000}),eventCount:events.length,events:Object.freeze(events),candles:Object.freeze(candles),coverage:Object.freeze({wallets:Object.freeze(coverages),complete,statement:coverageStatement}),verification:Object.freeze(verificationCounts),sources:Object.freeze([...sourceSet].sort()),caveats:Object.freeze(['Public-chain observations only; wallet relationships do not prove identity or common ownership.',window.startResolved?`Replay entry resolved from ${window.startSource}; retained market context begins up to seven days earlier when coverage exists.`:'Replay entry is unresolved; no synthetic lookback window was substituted.',quoteMint?'Execution price is shown only when indexed route evidence directly supports the selected base/quote pair.':'Execution quote is resolved separately from each retained direct swap leg; unknown USD sizes remain unavailable.',candles.length?'Candles are indexed market observations with their own confidence/source metadata.':'No indexed OHLC series is available for this selection yet; no price series was invented.','What-if overlays are historical counterfactuals, not predictions or claims of achievable execution.'])});
}
...
async function buildChainReplayBundle(db,subject,window,input){
  ...
  for(const wallet of wallets){const rows=await all(db.prepare(`SELECT event_id,chain_key,tx_id,wallet_address,asset_address,quote_asset_address,block_height,block_time,event_class,side,amount,price_usd,source,source_kind,confidence,evidence_json FROM intelligence_chain_events_v2 WHERE chain_key=? AND wallet_address=? AND asset_address=? AND block_time BETWEEN ? AND ? ORDER BY block_time ASC,COALESCE(block_height,0) ASC,event_id ASC LIMIT ?`).bind(chain,wallet,mint,from,to,limit));for(const row of rows){const event=normalizedChainReplayEvent(row);event.sources.forEach(source=>sourceSet.add(source));appendUniqueReplayEvent(events,event);}for(const event of await loadFomoReplayEvents(db,chain,wallet,mint,from,to)){event.sources.forEach(source=>sourceSet.add(source));appendUniqueReplayEvent(events,event);}}
  events.sort((a,b)=>a.timestamp-b.timestamp||(a.slot||0)-(b.slot||0)||String(a.id||'').localeCompare(String(b.id||'')));
  const marketContext=window.startResolved?await retainedExecutionContext(db,chain,wallets,mint,from,to):null,usd=!quoteMint||quoteMint===USD_QUOTE;let effectiveQuote=usd?USD_QUOTE:quoteMint,candleRows=window.startResolved&&effectiveQuote?await loadChainReplayCandleRows(db,chain,mint,effectiveQuote,bucketSeconds,from,to):[];if(window.startResolved&&!candleRows.length&&!usd){const retainedQuote=await retainedReplayQuote(db,chain,mint,from,to);if(retainedQuote&&retainedQuote!==effectiveQuote){effectiveQuote=retainedQuote;candleRows=await loadChainReplayCandleRows(db,chain,mint,effectiveQuote,bucketSeconds,from,to);}}
  const candles=candleRows.map(normalizeCandle);candles.flatMap(candle=>candle.sources||[]).forEach(source=>sourceSet.add(source));
  const referenceSeries=chain==='robinhood'?await loadRobinhoodReferenceSeries(db,mint,from,to):null;
  ...
  return Object.freeze({schemaVersion:'replay-bundle-v2',generatedAt:Date.now(),subject:Object.freeze({kind:wallets.length===2?'wallet-comparison':'wallet-token',chain,assetId:chainQualifiedId(chain,mint),wallets:Object.freeze(wallets),mint,quoteMint:effectiveQuote}),marketContext:marketContext?Object.freeze(marketContext):null,referenceSeries:referenceSeries&&referenceSeries.points?.length?referenceSeries:null,window:Object.freeze({...window,startTime:from*1000,endTime:to*1000}),eventCount:events.length,events:Object.freeze(events),candles:Object.freeze(candles),coverage:Object.freeze({wallets:Object.freeze(wallets.map(wallet=>Object.freeze({wallet,chain,status:'chain-qualified-retained-evidence'}))),complete:false,statement}),verification:Object.freeze(verification),sources:Object.freeze([...sourceSet].sort()),caveats:Object.freeze(['Provider-reported Fomo activity and independently observed chain receipts remain different evidence classes.','EVM Replay never enters the Solana history scheduler.',window.startResolved?`Replay entry resolved from ${window.startSource}; retained market context begins up to seven days earlier when coverage exists.`:'Replay entry is unresolved; no synthetic lookback window was substituted.',candles.length?(usd?'Candles are source-labeled USD market observations for this chain-qualified asset.':'Candles are source-labeled market observations for this chain-qualified asset.'):'No exact indexed OHLC pair is currently retained for this asset/window; provider-reported trade price points remain separately labeled and no chart path was invented.'])});
}
