 ...
 * companion backfill walks historical ranges from newest to oldest through
 * Blockscout's address+topic log API, so an empty database can recover verified
 * TokenLaunched events without scanning every Robinhood Chain block.
 */
...
// Conservative lower bounds. V1's published start block is 8,991,118. The
// V2 factory is first observed after block 26m; starting earlier is harmless.
...
async function fetchRange(factory,fromBlock,toBlock,fetchImpl=providerFetch,depth=0){
  const response=await fetchImpl(buildFilteredPonsLogsUrl(factory,fromBlock,toBlock),{headers:{accept:'application/json'}});
...
export async function backfillPonsLaunchDiscovery(env={},options={}){
  ...
  const chunkSize=clamp(options.chunkSize??env.PONS_DISCOVERY_CHUNK_BLOCKS,2_000_000,50_000,5_000_000);
  const chunksPerFactory=clamp(options.chunksPerFactory??env.PONS_DISCOVERY_CHUNKS_PER_RUN,4,1,8);
  ...
  for(const factory of PONS_FACTORIES){
    ...
    let to=Math.min(finalHead,Math.max(startBlock,n(state.next_to_block)||finalHead)),discovered=0,processed=0,error=null;
    for(let pass=0;pass<chunksPerFactory&&to>=startBlock;pass+=1){
      const from=Math.max(startBlock,to-chunkSize+1);
      try{
        const rows=await fetchRange(factory,from,to,options.fetchImpl||providerFetch);
        for(const row of rows)if(await persistLaunch(db,factory,row,now))discovered+=1;
        processed+=1;
        to=from-1;
    ...
    factories.push({factory:factory.address,version:factory.version,processed,complete:n(latest?.complete)===1,nextToBlock:latest?.next_to_block??null,discovered:n(latest?.discovered_launches),error:error||latest?.last_error||null});
  }
  const total=n((await db.prepare('SELECT COUNT(*) count FROM pons_launches').first())?.count);
  return Object.freeze({enabled:true,head,finalHead,chunkSize,chunksPerFactory,totalLaunches:total,factories});
}

export const __ponsDiscoveryContract=Object.freeze({readOnly:true,source:'blockscout-filtered-tokenlaunched',direction:'newest-to-oldest',resultLimit:RESULT_LIMIT});
