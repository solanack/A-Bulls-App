...
export function summarizeIndexJobs(jobs=[],nowSeconds=Math.floor(Date.now()/1000)){
  ...
  return Object.freeze({complete,running,queued,waitingExternal,retrying,nextRetryAt,pagesCompleted,signaturesSeen,transactionsIngested,sources});
}
...
export async function readIndexJobStatus(env={},input={}){
  ...
  return Object.freeze({ok:true,schedulerEnabled:schedulerEnabled(env),requested:ids.length,found:jobs.length,...summary,jobs:Object.freeze(jobs),disclosure:'Job status reports bounded read-only index progress only. A waiting-external job means the Worker delegated a bounded public-wallet retrieval task to an approved external bridge; its listed source is planned until evidence is actually ingested. Source names are not provider endorsements or completeness claims. Page, signature and transaction counts describe work performed by these jobs, not token-market totals. A complete job still does not establish complete token-market coverage.'});
}
