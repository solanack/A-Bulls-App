...
export function normalizeMarketReplayEvent(row={}){
  ...
  const delta=n(row.token_delta);
  const side=swapLike?(delta>0?'buy':delta<0?'sell':'trade'):s(row.event_class)||'event';
  ...
  return Object.freeze({
    id:s(row.signature)||`${s(row.wallet)}-${Math.trunc(n(row.block_time))}`,
    ...
    timestamp:Math.max(0,Math.trunc(n(row.block_time)))*1000,
...
function candle(row={}){
  ...
  return Object.freeze({timestamp:Math.max(0,Math.trunc(n(row.bucket_start)))*1000,bucketSeconds:Math.max(60,Math.trunc(n(row.bucket_seconds)||60)),open:n(row.open),high:n(row.high),low:n(row.low),close:n(row.close),volumeBase:n(row.volume_base),volumeQuote:n(row.volume_quote),swapCount:Math.max(0,Math.trunc(n(row.swap_count))),walletCount:Math.max(0,Math.trunc(n(row.wallet_count))),confidence:clamp(row.confidence,0,1),sources:Object.freeze(uniq(sources))});
}
...
export async function buildMarketReplayBundle(env={},input={}){
  ...
  const events=rows.map(normalizeMarketReplayEvent);
  ...
  const candles=Object.freeze(candleRows.map(candle));
