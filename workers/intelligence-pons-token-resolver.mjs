...
export async function resolveRobinhoodToken(address,{env={},fetchImpl=fetch}={}){
  ...
  if(rpcAvailable&&(!code||code==='0x'||code==='0x0'))return Object.freeze({ok:true,kind:'evm-token',address:token,state:'not-found',label:'unresolved-robinhood-address',chainId:4663,network:'Robinhood Chain',source:'robinhood-chain-rpc',readOnly:true});
