...
test('balance activeTrade metadata preserves a provider-reported Replay anchor and entry price',()=>{
  const captured=1_790_000_000_000,rows=normalizeFomoHoldings({holdings:[{token:{symbol:'BULL',address:'0x2222222222222222222222222222222222222222'},chain:'base',amount:'4',activeTrade:{id:'active-1',createdAt:1_789_000_000,avgEntryPrice:'1.25',status:'open'}}]},captured);
  assert.equal(rows.length,1);
  assert.equal(rows[0].activeTradeCreatedAt,1_789_000_000);
  assert.equal(rows[0].activeTradeEntryPrice,1.25);
  const anchors=holdingTradeAnchors(rows,captured);
  assert.equal(anchors.length,1);
  assert.equal(anchors[0].tradeId,'active-1');
  assert.equal(anchors[0].createdAt,1_789_000_000);
  assert.equal(anchors[0].avgEntryPrice,1.25);
  assert.equal(anchors[0].source,'fomoapi.io/balances-active-trade');
});

test('trade timestamps accept epoch seconds and documented price field variants',()=>{
  const rows=normalizeFomoTrades({trades:[{tradeId:'t2',tokenAddress:'0x3333333333333333333333333333333333333333',chain:'bsc',createdAt:1_789_000_000,closedAt:1_789_100_000,entryPriceUsd:'0.5',exitPriceUsd:'0.75'}]});
  assert.equal(rows[0].createdAt,1_789_000_000);
