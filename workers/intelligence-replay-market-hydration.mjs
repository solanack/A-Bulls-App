...
function leaseKey({chain,mint,quoteMint,poolAddress,bucketSeconds,from,to}){return`replay-market:v6:${s(chain)}:${s(mint)}:${s(quoteMint)}:${s(poolAddress)}:${Math.trunc(n(bucketSeconds)||60)}:${Math.trunc(n(from))}:${Math.trunc(n(to))}`;}
...
async function fetchPoolWindow(env,pool,mint,from,to,spec,fetchImpl,currency='token'){
...
async function retainedAnyCandleCount(db,chain,mint,from,to){
  const row=chain==='solana'?await first(db.prepare(`SELECT COUNT(*) count FROM intelligence_price_candles_v2 WHERE chain_key=? AND asset_address=? AND bucket_start BETWEEN ? AND ?`).bind(chain,mint,from,to)):await first(db.prepare(`SELECT COUNT(*) count FROM intelligence_price_candles_v2 WHERE chain_key=? AND asset_address=? AND quote_asset_address=? AND bucket_start BETWEEN ? AND ?`).bind(chain,mint,USD_QUOTE,from,to));return Math.max(0,Math.trunc(n(row?.count)));
}
export async function prewarmFomoReplayCandles(env={},nowMs=Date.now(),{fetchImpl=providerFetch}={}){
  ...
  const now=Math.floor(nowMs/1000),seen=new Set(),results=[];let scanned=0,alreadyReady=0,cooldownSkipped=0;
  for(const row of rows){
    if(results.length>=limit)break;scanned+=1;
    ...
    const execution=await first(db.prepare(`SELECT quote_asset_address,pool_address FROM intelligence_trade_execution_v2 WHERE chain_key=? AND asset_address=? AND block_time BETWEEN ? AND ? ORDER BY confidence DESC,block_time ASC LIMIT 1`).bind(chain,mint,from,to)),quoteMint=chain==='solana'?canonicalChainAddress(chain,execution?.quote_asset_address)||'':'',poolAddress=canonicalProviderPoolId(chain,execution?.pool_address)||'',bucketSeconds=chooseReplayPrewarmBucket(from,to),lease=await leaseRow(db,leaseKey({chain,mint,quoteMint,poolAddress,bucketSeconds,from,to})),leaseState=s(lease?.last_state),leaseUpdated=Math.trunc(n(lease?.updated_at));
