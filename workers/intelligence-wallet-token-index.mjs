...
export async function handleWalletTokenIndexRequest(request,env={}){
  ...
  try{if(compareWallet){const comparison=await buildCommonTokenIndex(env,wallet,compareWallet,{limit});await recordDemand(env,'wallet-common-token-index','wallet-comparison',`${wallet}:${compareWallet}`,Date.now()-started).catch(()=>null);return json({ok:true,comparison});}const index=await buildWalletTokenIndex(env,wallet,{limit});await recordDemand(env,'wallet-token-index','wallet',wallet,Date.now()-started).catch(()=>null);return json({ok:true,index});}
