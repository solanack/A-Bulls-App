...
export async function buildCommonTokenIndex(env={},walletAInput='',walletBInput='',{limit=100,db:dbOverride}={}){
  ...
  const common=a.tokens.filter(item=>byB.has(item.mint)).map(item=>{const other=byB.get(item.mint);return Object.freeze({mint:item.mint,walletA:Object.freeze(item),walletB:Object.freeze(other),combinedTradeCount:item.tradeCount+other.tradeCount,combinedEventCount:item.eventCount+other.eventCount,overlapFrom:Math.max(item.firstEvent||0,other.firstEvent||0)||null,overlapTo:Math.min(item.lastEvent||Infinity,other.lastEvent||Infinity)});}).filter(item=>item.overlapFrom==null||item.overlapTo==null||item.overlapFrom<=item.overlapTo).sort((x,y)=>y.combinedTradeCount-x.combinedTradeCount||y.combinedEventCount-x.combinedEventCount).slice(0,Math.max(1,Math.min(250,Math.trunc(n(limit)||100))));
  return Object.freeze({schemaVersion:'wallet-common-token-index-v1',walletA,walletB,commonTokenCount:common.length,commonTokens:Object.freeze(common),coverage:Object.freeze({walletA:a.coverage,walletB:b.coverage}),disclosure:'Common tokens mean both addresses have indexed observations for the same mint. This does not prove shared ownership, coordination, strategy, or complete historical overlap.'});
}
...
export async function handleWalletTokenIndexRequest(request,env={}){
  ...
  try{
    if(compareWallet){const comparison=await buildCommonTokenIndex(env,wallet,compareWallet,{limit});await recordDemand(env,'wallet-common-token-index','wallet-comparison',`${wallet}:${compareWallet}`,Date.now()-started).catch(()=>null);return json({ok:true,comparison});}
    const index=await buildWalletTokenIndex(env,wallet,{limit});await recordDemand(env,'wallet-token-index','wallet',wallet,Date.now()-started).catch(()=>null);return json({ok:true,index});
