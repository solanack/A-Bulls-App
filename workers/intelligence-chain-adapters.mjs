...
export async function discoverGoldRushWalletTokenActivity(env={},input={}, {fetchImpl=providerFetch}={}){
  ...
  const deduped=[...new Map(candidates.map(row=>[row.txId,row])).values()];const selected=selectReconciliationCandidate(deduped,{at,side:input.side,maxWindowSeconds:windowSeconds});return selected?Object.freeze({ok:true,state:'matched',chain,wallet,token,...selected}):Object.freeze({ok:true,state:deduped.length?'ambiguous-or-direction-mismatch':'unmatched',chain,wallet,token,candidates:deduped.length,source:'goldrush-transactions-v3'});
}
