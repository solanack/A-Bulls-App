...
async function traderTradesPayload(env,handle){
  const preferred=effectiveTradeFetchLimit??clamp(env.FOMO_TRADE_FETCH_LIMIT,100,25,100);
  try{const payload=await apiJson(env,`/users/${encodeURIComponent(handle)}/trades?limit=${preferred}`,1);effectiveTradeFetchLimit=preferred;return payload;}
  catch(error){const code=s(error?.message||error);if(preferred<=25||!/^fomoapi_http_(400|404|422)$/.test(code))throw error;effectiveTradeFetchLimit=25;return apiJson(env,`/users/${encodeURIComponent(handle)}/trades?limit=25`,1);}
}
...
export function normalizeFomoHoldings(payload,capturedAt=Date.now()){
  return payloadArray(payload,'holdings','data.holdings','balances','data.balances').flatMap(item=>{
    const token=item?.token&&typeof item.token==='object'?item.token:{},userToken=item?.userToken&&typeof item.userToken==='object'?item.userToken:{},active=item?.activeTrade&&typeof item.activeTrade==='object'?item.activeTrade:item?.position?.activeTrade&&typeof item.position.activeTrade==='object'?item.position.activeTrade:{};
    const address=s(token.address??item?.tokenAddress??item?.token_address??item?.address);if(!address)return[];
    const valueUsd=finite(item?.valueUsd??item?.value_usd),amount=finite(item?.amount??item?.balance),priceUsd=finite(item?.priceUsd??item?.price_usd),activeTradeCreatedAt=parseSec(active?.createdAt??active?.openedAt??active?.created_at??active?.opened_at),activeTradeClosedAt=parseSec(active?.closedAt??active?.closed_at),activeTradeEntryPrice=firstFinite(active?.avgEntryPrice,active?.avg_entry_price,active?.entryPriceUsd,active?.entry_price_usd,active?.averageEntryPriceUsd,active?.average_entry_price_usd,userToken?.averageEntryPriceUsd,userToken?.average_entry_price_usd,item?.averageEntryPriceUsd,item?.average_entry_price_usd),activeTradeExitPrice=firstFinite(active?.avgExitPrice,active?.avg_exit_price,active?.exitPriceUsd,active?.exit_price_usd,active?.averageExitPriceUsd,active?.average_exit_price_usd);
    return [{tokenAddress:address,symbol:s(token.symbol??item?.symbol).slice(0,32)||null,name:s(token.name??item?.name).slice(0,120)||null,chain:s(item?.chain??token?.chain??active?.chain).slice(0,32)||null,networkId:s(token.networkId??item?.networkId??item?.network_id??active?.networkId??active?.network_id).slice(0,32)||null,amount,priceUsd,valueUsd,change24h:finite(item?.change24h??item?.change_24h),activeTradeId:s(active?.tradeId??active?.trade_id??active?.id).slice(0,180)||null,activeTradeCreatedAt,activeTradeClosedAt,activeTradeEntryPrice,activeTradeExitPrice,activeTradeAmount:firstFinite(active?.amount,amount),activeTradeStatus:s(active?.status).toLowerCase()||null,capturedAt}];
...
export function normalizeFomoTrades(payload,capturedAt=Date.now()){
  return payloadArray(payload,'trades','data.trades','items','data.items','results','data.results','data').flatMap((item,index)=>{
    const token=item?.token&&typeof item.token==='object'?item.token:{};const address=s(token.address??item?.tokenAddress??item?.token_address??item?.address);if(!address)return[];
    const createdAt=parseSec(item?.createdAt??item?.openedAt??item?.created_at??item?.opened_at??item?.entryAt??item?.entry_at),closedAt=parseSec(item?.closedAt??item?.closed_at??item?.exitedAt??item?.exited_at);
    return [{tradeId:s(item?.tradeId??item?.trade_id??item?.id??`${address}:${createdAt??capturedAt}:${index}`).slice(0,180),tokenAddress:address,symbol:s(token.symbol??item?.symbol).slice(0,32)||null,chain:s(item?.chain??token?.chain??item?.network).slice(0,32)||null,status:s(item?.status).toLowerCase()==='closed'||closedAt>0?'closed':'open',amount:finite(item?.amount),avgEntryPrice:firstFinite(item?.avgEntryPrice,item?.avg_entry_price,item?.entryPriceUsd,item?.entry_price_usd,item?.averageEntryPriceUsd,item?.average_entry_price_usd,item?.entryPrice,item?.entry_price),avgExitPrice:firstFinite(item?.avgExitPrice,item?.avg_exit_price,item?.exitPriceUsd,item?.exit_price_usd,item?.averageExitPriceUsd,item?.average_exit_price_usd,item?.exitPrice,item?.exit_price),realizedPnlUsd:finite(item?.realizedPnlUsd??item?.realized_pnl_usd),unrealizedPnlUsd:finite(item?.unrealizedPnlUsd??item?.unrealized_pnl_usd),createdAt,closedAt,capturedAt,source:'fomoapi.io/trades'}];
  }).sort((a,b)=>Math.max(n(b.closedAt),n(b.createdAt))-Math.max(n(a.closedAt),n(a.createdAt))).slice(0,250);
}

export function holdingTradeAnchors(holdings=[],capturedAt=Date.now()){
  return (Array.isArray(holdings)?holdings:[]).flatMap(item=>{const createdAt=Math.max(0,Math.trunc(n(item?.activeTradeCreatedAt))),tokenAddress=s(item?.tokenAddress);if(!createdAt||!tokenAddress)return[];const closedAt=Math.max(0,Math.trunc(n(item?.activeTradeClosedAt))),status=s(item?.activeTradeStatus).toLowerCase()==='closed'||closedAt>=createdAt&&closedAt>0?'closed':'open',tradeId=s(item?.activeTradeId)||`balance:${tokenAddress}:${createdAt}`;return[{tradeId,tokenAddress,symbol:s(item?.symbol)||null,chain:s(item?.chain)||null,status,amount:finite(item?.activeTradeAmount),avgEntryPrice:finite(item?.activeTradeEntryPrice),avgExitPrice:finite(item?.activeTradeExitPrice),realizedPnlUsd:null,unrealizedPnlUsd:null,createdAt,closedAt:closedAt||null,capturedAt,source:'fomoapi.io/balances-active-trade'}];});
}
...
async function storeTrades(db,handle,trades,now){
  for(const item of trades)await db.prepare(`INSERT INTO fomo_trader_trades(handle,trade_id,token_address,symbol,chain,status,amount,avg_entry_price,avg_exit_price,realized_pnl_usd,unrealized_pnl_usd,created_at,closed_at,captured_at,source) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(handle,trade_id) DO UPDATE SET token_address=excluded.token_address,symbol=COALESCE(excluded.symbol,fomo_trader_trades.symbol),chain=COALESCE(excluded.chain,fomo_trader_trades.chain),status=excluded.status,amount=COALESCE(excluded.amount,fomo_trader_trades.amount),avg_entry_price=COALESCE(excluded.avg_entry_price,fomo_trader_trades.avg_entry_price),avg_exit_price=COALESCE(excluded.avg_exit_price,fomo_trader_trades.avg_exit_price),realized_pnl_usd=COALESCE(excluded.realized_pnl_usd,fomo_trader_trades.realized_pnl_usd),unrealized_pnl_usd=COALESCE(excluded.unrealized_pnl_usd,fomo_trader_trades.unrealized_pnl_usd),created_at=COALESCE(excluded.created_at,fomo_trader_trades.created_at),closed_at=COALESCE(excluded.closed_at,fomo_trader_trades.closed_at),captured_at=excluded.captured_at,source=CASE WHEN fomo_trader_trades.source='fomoapi.io/trades' THEN fomo_trader_trades.source ELSE excluded.source END`).bind(handle,item.tradeId,item.tokenAddress,item.symbol,item.chain,item.status,item.amount,item.avgEntryPrice,item.avgExitPrice,item.realizedPnlUsd,item.unrealizedPnlUsd,item.createdAt,item.closedAt,now,s(item.source)||'fomoapi.io/trades').run();
  await db.prepare("DELETE FROM fomo_trader_trades WHERE handle=? AND captured_at<? AND status<>'closed'").bind(handle,now-30*86400).run();
}
async function enrichTrader(env,db,handle,now){
  try{
    const [balances,trades]=await Promise.all([apiJson(env,`/users/${encodeURIComponent(handle)}/balances`,1),traderTradesPayload(env,handle)]);
    const holdings=normalizeFomoHoldings(balances,now*1000),history=normalizeFomoTrades(trades,now*1000),anchors=holdingTradeAnchors(holdings,now*1000);
...
async function backfillCurrentFomoCoverage(env,db,now){const limit=clamp(env.FOMO_COVERAGE_BACKFILL_LIMIT,10,0,10),targets=await currentEnrichmentTargets(db,limit,{missingOnly:true});return enrichTargets(env,db,targets,now);}
...
export async function refreshFomoLive(env={},nowMs=Date.now()){
  ...
  if(n(state?.last_fetch_at)&&now-n(state.last_fetch_at)<refreshMinutes*60){const coverageBackfill=await backfillCurrentFomoCoverage(env,db,now);return Object.freeze({enabled:true,configured:true,skipped:'fresh-cache',coverageBackfill,lastSuccessAt:n(state?.last_success_at)||null});}
  ...
  try{
    ...
    const positionCount=n((await db.prepare('SELECT COUNT(*) count FROM fomo_trader_positions').first().catch(()=>null))?.count),missing=await backfillCurrentFomoCoverage(env,db,now),steadyLimit=positionCount===0?clamp(env.FOMO_BOOTSTRAP_ENRICH_LIMIT,10,0,10):clamp(env.FOMO_ENRICH_TRADERS_PER_REFRESH,1,0,2),steadyTargets=missing.length?[]:await currentEnrichmentTargets(db,steadyLimit),enriched=missing.length?missing:await enrichTargets(env,db,steadyTargets,now);
...
export async function handleFomoLiveRequest(request,env={}){
  ...
  const handle=s(url.searchParams.get('handle')).replace(/^@/,'');if(!handle)return null;const traderRow=await db.prepare('SELECT * FROM fomo_traders WHERE handle=? COLLATE NOCASE AND captured_at=(SELECT MAX(captured_at) FROM fomo_traders) LIMIT 1').bind(handle).first().catch(()=>null);if(!traderRow)return null;
  ...
  const providerTrades=tradeRows.map(row=>{const closed=n(row.closed_at),created=n(row.created_at),isExit=closed>0&&closed>=created,observedAt=(isExit?closed:created)*1000;return{signature:null,wallet:trader.solanaWallet??trader.evmWallet??trader.handle,mint:s(row.token_address),chain:s(row.chain)||null,side:isExit?'sell':'buy',solAmount:0,tokenAmount:Math.abs(n(row.amount)),priceSol:null,priceUsd:finite(isExit?row.avg_exit_price:row.avg_entry_price),status:s(row.status)||null,realizedPnlUsd:finite(row.realized_pnl_usd),unrealizedPnlUsd:finite(row.unrealized_pnl_usd),observedAt,source:'fomoapi.io/trades',sourceKind:'fomo-reported-position-event'};}).filter(item=>item.mint&&item.observedAt>0);
...
export const __fomoLiveContract=Object.freeze({leaderboardPath:'/leaderboard/all?limit=50',maximumTraders:50,maximumPositions:10,providerTradeFetchDefault:100,providerTradeFetchFallback:25,closedTradeRetention:'durable',latestTrades:3,pageReadsProviderFree:true,scheduled:true,requiresApiKey:true,bootstrapEnrichmentMax:10,priorityDetailedTraders:50,coverageBackfillMax:10,activeTradeAnchors:true});
