...
import { findWalletTokenActivity,resolveEvmVenuePool } from './intelligence-chain-adapters.mjs';
...
function executionUpsert(db,row){return db.prepare(`INSERT INTO intelligence_trade_execution_v2(chain_key,tx_id,wallet_address,asset_address,quote_asset_address,side,pool_address,dex_id,block_height,block_time,source,confidence,evidence_json,observed_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,unixepoch(),unixepoch()) ON CONFLICT(chain_key,tx_id,wallet_address,asset_address) DO UPDATE SET quote_asset_address=COALESCE(excluded.quote_asset_address,intelligence_trade_execution_v2.quote_asset_address),side=COALESCE(excluded.side,intelligence_trade_execution_v2.side),pool_address=COALESCE(excluded.pool_address,intelligence_trade_execution_v2.pool_address),dex_id=COALESCE(excluded.dex_id,intelligence_trade_execution_v2.dex_id),block_height=COALESCE(excluded.block_height,intelligence_trade_execution_v2.block_height),block_time=COALESCE(excluded.block_time,intelligence_trade_execution_v2.block_time),source=excluded.source,confidence=MAX(intelligence_trade_execution_v2.confidence,excluded.confidence),evidence_json=excluded.evidence_json,updated_at=unixepoch()`).bind(row.chain,row.txId,row.walletAddress,row.assetAddress,row.quoteAssetAddress||null,row.side||null,row.poolAddress||null,row.dexId||null,row.blockHeight||null,row.blockTime||null,row.source,row.confidence||0,JSON.stringify(row.evidence||{}));}
...
export async function verifyEvmTransaction(env,event,{fetchImpl=providerFetch}={}){
  const txId=candidateTransactionId(event?.chain,event?.txId??event?.evidence?.tradeId);if(!txId)return null;const definition=resolveChain(event.chain,{address:event.assetAddress});if(definition?.kind!=='evm')return null;
  const candidates=rpcCandidatesForChain(env,event.chain,event.assetAddress);for(const candidate of candidates){try{const [tx,receipt]=await Promise.all([evmRpc(candidate.url,'eth_getTransactionByHash',[txId],fetchImpl),evmRpc(candidate.url,'eth_getTransactionReceipt',[txId],fetchImpl)]);if(!tx||!receipt)continue;const from=canonicalChainAddress(event.chain,tx.from),to=canonicalChainAddress(event.chain,tx.to),blockNumber=s(receipt.blockNumber??tx.blockNumber),blockHeight=hexInt(blockNumber),block=blockNumber?await evmRpc(candidate.url,'eth_getBlockByNumber',[blockNumber,false],fetchImpl).catch(()=>null):null,blockTime=hexInt(block?.timestamp),success=s(receipt.status).toLowerCase()==='0x1';return Object.freeze({eventId:`receipt:${event.chain}:${txId}`,chain:event.chain,txId,walletAddress:from,assetAddress:event.assetAddress,quoteAssetAddress:null,blockHeight,blockTime,eventClass:'transaction-receipt',side:null,amount:null,priceUsd:null,source:candidate.source,sourceKind:'observed-fact',confidence:0.9,evidence:Object.freeze({to,success,status:s(receipt.status)||null,gasUsed:s(receipt.gasUsed)||null,logsCount:Array.isArray(receipt.logs)?receipt.logs.length:null,tradeInterpretationVerified:false}),receipt});}catch{}}
  return null;
}
...
export async function materializeFomoMultichainEvidence(env={},nowMs=Date.now(),{fetchImpl=providerFetch}={}){
  ...
  let verified=0,attempted=0,executionContexts=0;for(const event of events){if(attempted>=verifyLimit)break;if(!event.txId)continue;const definition=resolveChain(event.chain,{address:event.assetAddress});if(definition?.kind!=='evm')continue;attempted+=1;const receipt=await verifyEvmTransaction(env,event,{fetchImpl});if(receipt){await eventUpsert(db,receipt).run();const venue=await resolveEvmVenuePool(env,{chain:event.chain,token:event.assetAddress,receipt:receipt.receipt},{fetchImpl}).catch(()=>null);const wallet=canonicalChainAddress(event.chain,event.walletAddress)||receipt.walletAddress,side=event.side==='entry'?'buy':event.side==='exit'?'sell':event.side;if(wallet){await executionUpsert(db,{chain:event.chain,txId:receipt.txId,walletAddress:wallet,assetAddress:event.assetAddress,quoteAssetAddress:venue?.quoteAssetAddress||null,side,poolAddress:venue?.poolAddress||null,dexId:venue?.dexId||null,blockHeight:receipt.blockHeight,blockTime:receipt.blockTime,source:venue?.source||receipt.source,confidence:venue?0.95:0.9,evidence:{providerEventId:event.eventId,providerPhase:event.evidence?.phase||null,receiptVerified:true,poolMatched:Boolean(venue)}}).run();executionContexts+=1;}verified+=1;}}
  const reconcileLimit=clamp(env.MULTICHAIN_TX_RECONCILE_LIMIT,2,0,8),reconciliationEvents=selectReconciliationEvents(events,reconcileLimit,nowMs);let reconciled=0,reconcileAttempted=0;const reconciliationStates={};
  for(const event of reconciliationEvents){const definition=resolveChain(event.chain,{address:event.assetAddress});if(!definition){reconciliationStates.unsupported=(reconciliationStates.unsupported||0)+1;continue;}reconcileAttempted+=1;const match=await findWalletTokenActivity(env,{chain:event.chain,wallet:event.walletAddress,token:event.assetAddress,at:event.blockTime,side:event.side,windowSeconds:clamp(env.MULTICHAIN_TX_RECONCILE_WINDOW_SECONDS,900,60,3600)},{fetchImpl}).catch(error=>({state:'error',reason:s(error?.message||error)}));const matchState=s(match?.state)||'unavailable';reconciliationStates[matchState]=(reconciliationStates[matchState]||0)+1;if(!match||match.state!=='matched')continue;
    const txId=s(match.txId||match.signature);if(!txId){reconciliationStates['matched-without-tx']=(reconciliationStates['matched-without-tx']||0)+1;continue;}let venue=null;if(definition.kind==='evm'&&match.receipt)venue=await resolveEvmVenuePool(env,{chain:event.chain,token:event.assetAddress,receipt:match.receipt},{fetchImpl}).catch(()=>null);
    const side=event.side==='entry'?'buy':event.side==='exit'?'sell':event.side,observed={eventId:`reconciled:${event.chain}:${txId}:${event.assetAddress}`,chain:event.chain,txId,walletAddress:canonicalChainAddress(event.chain,event.walletAddress),assetAddress:event.assetAddress,quoteAssetAddress:venue?.quoteAssetAddress||null,blockHeight:match.blockNumber??match.slot??null,blockTime:match.blockTime,eventClass:'reconciled-wallet-token-activity',side,amount:event.amount,priceUsd:null,source:s(match.source)||'chain-reconciliation',sourceKind:'observed-fact',confidence:Number(match.confidence)||.82,evidence:{providerEventId:event.eventId,providerPhase:event.evidence?.phase||null,timeDeltaSeconds:match.timeDeltaSeconds??null,directionObserved:true,poolMatched:Boolean(venue),tradeInterpretationVerified:Boolean(venue)}};
...
async function statusPayload(env){
  ...
  const [assets,markets,events,kinds,executions,reconciliationHealth]=await Promise.all([
    ...
    all(db.prepare(`SELECT chain_key,COUNT(*) count,SUM(CASE WHEN TRIM(COALESCE(dex_id,''))<>'' THEN 1 ELSE 0 END) venue_count,SUM(CASE WHEN TRIM(COALESCE(pool_address,''))<>'' THEN 1 ELSE 0 END) pool_count,MAX(block_time) last_observed_at FROM intelligence_trade_execution_v2 GROUP BY chain_key ORDER BY chain_key`)),
    first(db.prepare(`SELECT state,last_ok_at,last_error_at,details_json,updated_at FROM intelligence_source_health WHERE source='multichain-reconciliation'`))
  ]);
  return{ok:true,schemaVersion:'multichain-evidence-spine-v1',enabled:bool(env.MULTICHAIN_EVIDENCE_ENABLED),marketEnabled:bool(env.MULTICHAIN_MARKET_ENABLED),fomoCoverageTargets:fomoChainTargets(),assets,markets,events,evidenceKinds:kinds,executionContexts:executions,reconciliationHealth:reconciliationHealth||null,disclosure:'Chain-qualified caches keep Fomo provider reports separate from independently observed transaction receipts. Missing rows mean unavailable coverage, not zero activity. No signing, swaps, custody, or execution are performed.'};
}
