...
export function fomoTradeToChainEvent(row={}){
  const chain=rowChain(row),definition=resolveChain(chain,{address:row.token_address}),assetAddress=canonicalChainAddress(chain,row.token_address);if(!definition||!assetAddress)return null;
  const createdAt=Math.max(0,Math.trunc(n(row.created_at))),closedAt=Math.max(0,Math.trunc(n(row.closed_at))),closed=closedAt>0&&closedAt>=createdAt,blockTime=closed?closedAt:createdAt,wallet=walletForRow(row,definition);
  const tradeId=s(row.trade_id);if(!tradeId||!blockTime)return null;
  const txId=candidateTransactionId(chain,row.tx_id??tradeId),price=finite(closed?row.avg_exit_price:row.avg_entry_price);
  return Object.freeze({eventId:`fomo:${chain}:${s(row.handle).toLowerCase()}:${tradeId}`,chain,txId,walletAddress:wallet||null,assetAddress,quoteAssetAddress:null,blockHeight:null,blockTime,eventClass:closed?'fomo-position-exit':'fomo-position-entry',side:closed?'exit':'entry',amount:finite(row.amount),priceUsd:price,source:'fomoapi.io/trades',sourceKind:'provider-reported',confidence:0.65,evidence:Object.freeze({handle:s(row.handle),tradeId,status:s(row.status)||null,reportedRealizedPnlUsd:finite(row.realized_pnl_usd),reportedUnrealizedPnlUsd:finite(row.unrealized_pnl_usd)})});
}
...
export async function materializeFomoMultichainEvidence(env={},nowMs=Date.now(),{fetchImpl=providerFetch}={}){
  ...
  const events=trades.map(fomoTradeToChainEvent).filter(Boolean),writes=[];for(const asset of assetsById.values())writes.push(assetUpsert(db,asset));for(const event of events)writes.push(eventUpsert(db,event));for(let i=0;i<writes.length;i+=50)await db.batch(writes.slice(i,i+50));
...
export const __multichainEvidenceContract=Object.freeze({statusPath:STATUS_PATH,schemaVersion:'multichain-evidence-spine-v1',readOnly:true,pageReadsProviderFree:true,providerReportedTradeFactsStayProviderReported:true,receiptVerificationDoesNotVerifyTradeInterpretation:true});
